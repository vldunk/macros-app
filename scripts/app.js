        ...
        const LOCAL_FOOD_SEARCH_PRODUCTS = [
            { id: 'local_rice_white', name: 'Рис', caloriesPer100: 344, proteinPer100: 6.7, fatPer100: 0.7, carbsPer100: 78.9 },
            { id: 'local_rice_jasmine', name: 'Рис жасмин', caloriesPer100: 350, proteinPer100: 7.0, fatPer100: 0.6, carbsPer100: 79.0 },
            { id: 'local_rice_pilaf', name: 'Рис для плова', caloriesPer100: 345, proteinPer100: 7.0, fatPer100: 0.8, carbsPer100: 78.0 },
            { id: 'local_rice_basmati', name: 'Рис басмати', caloriesPer100: 347, proteinPer100: 7.5, fatPer100: 0.7, carbsPer100: 77.7 },
            { id: 'local_rice_groats', name: 'Крупа рисовая', caloriesPer100: 330, proteinPer100: 7.0, fatPer100: 1.0, carbsPer100: 74.0 },
            { id: 'local_milk_25', name: 'Молоко 2.5%', caloriesPer100: 53, proteinPer100: 3.0, fatPer100: 2.5, carbsPer100: 4.7 },
            { id: 'local_milk_15', name: 'Молоко 1.5%', caloriesPer100: 44, proteinPer100: 3.0, fatPer100: 1.5, carbsPer100: 4.8 },
            { id: 'local_cottage_cheese_5', name: 'Творог 5%', caloriesPer100: 121, proteinPer100: 17.0, fatPer100: 5.0, carbsPer100: 2.0 },
            { id: 'local_cottage_cheese_2', name: 'Творог 2%', caloriesPer100: 103, proteinPer100: 18.0, fatPer100: 2.0, carbsPer100: 3.3 },
            { id: 'local_chicken', name: 'Курица', caloriesPer100: 190, proteinPer100: 18.2, fatPer100: 12.6, carbsPer100: 0 },
            { id: 'local_chicken_breast', name: 'Куриная грудка', caloriesPer100: 113, proteinPer100: 23.6, fatPer100: 1.9, carbsPer100: 0.4 },
            { id: 'local_chicken_fillet', name: 'Куриное филе', caloriesPer100: 110, proteinPer100: 23.1, fatPer100: 1.2, carbsPer100: 0 }
        ];
        let diaryFoodSearchDebounceId = 0;
        let diaryFoodSearchController = null;
        let diaryFoodSearchState = { query: '', status: 'idle', results: [], requestId: 0 };
        let quickAddLongPressTimer = 0;
        let quickAddLongPressHandled = false;
        ...
        function quickEntriesStorageKey() {
            return 'quickEntries_' + appUserId;
        }

        function loadQuickEntries() {
            try { return JSON.parse(localStorage.getItem(quickEntriesStorageKey())) || []; } catch (e) { return []; }
        }

        function saveQuickEntries(entries) {
            localStorage.setItem(quickEntriesStorageKey(), JSON.stringify(Array.isArray(entries) ? entries : []));
        }
        ...
        function formatManualProductMacros(product) {
            return Math.round(Number(product.caloriesPer100) || 0) + ' ккал • Б ' +
                Math.round(Number(product.proteinPer100) || 0) + ' • Ж ' +
                Math.round(Number(product.fatPer100) || 0) + ' • У ' +
                Math.round(Number(product.carbsPer100) || 0);
        }
        ...
        function smartUxStorageKey(suffix) {
            return 'mypie_smart_' + suffix + '_' + appUserId;
        }

        function loadSmartJson(suffix, fallback) {
            try {
                const value = JSON.parse(localStorage.getItem(smartUxStorageKey(suffix)));
        ...
        function saveSmartJson(suffix, value) {
            localStorage.setItem(smartUxStorageKey(suffix), JSON.stringify(value));
        }

        function getLastSmartMealType(fallback = 'Обед') {
            return getValidMealType(loadSmartJson('lastMealType', ''), getValidMealType(fallback, 'Обед'));
        }

        function rememberSmartMealType(mealType) {
            saveSmartJson('lastMealType', getValidMealType(mealType, 'Обед'));
        }

        function getSmartDefaultMealType(fallback = 'Обед') {
            if (isDiaryMealScreenOpen()) return getValidMealType(currentDiaryMealType, fallback);
            return getLastSmartMealType(currentMealFilter || fallback);
        }

        function getRecipeUsageMap() {
            return loadSmartJson('recipeUsage', {});
        }

        function saveRecipeUsageMap(map) {
            saveSmartJson('recipeUsage', map && typeof map === 'object' ? map : {});
        }

        function getProductUsageMap() {
            return loadSmartJson('productUsage', {});
        }

        function saveProductUsageMap(map) {
            saveSmartJson('productUsage', map && typeof map === 'object' ? map : {});
        }

        function recordRecipeSmartAction(recipeId, mealType, action = 'serving') {
            const id = String(recipeId || '');
            if (!id) return;
            const map = getRecipeUsageMap();
            const item = map[id] || { count: 0 };
            map[id] = {
                ...item,
                count: Number(item.count) + 1,
                lastUsedAt: Date.now(),
                lastMealType: getValidMealType(mealType, 'Обед'),
                lastAction: action || 'serving'
            };
            saveRecipeUsageMap(map);
            rememberSmartMealType(mealType);
        }

        function recordProductSmartAction(productId, mealType) {
            const id = String(productId || '');
            if (!id) return rememberSmartMealType(mealType);
            const map = getProductUsageMap();
            const item = map[id] || { count: 0 };
            map[id] = {
                ...item,
                count: Number(item.count) + 1,
                lastUsedAt: Date.now(),
                lastMealType: getValidMealType(mealType, 'Перекус')
            };
            saveProductUsageMap(map);
            rememberSmartMealType(mealType);
        }

        function isFavoriteRecipeId(recipeId) {
            try {
                return (JSON.parse(localStorage.getItem('fav_recipes_' + appUserId)) || []).map(String).includes(String(recipeId));
            } catch (e) {
                return false;
            }
        }

        function getDefaultRecipeQuickAction(recipeId) {
            const usage = getRecipeUsageMap()[String(recipeId || '')];
            return isFavoriteRecipeId(recipeId) && usage?.lastAction ? usage.lastAction : 'serving';
        }

        function applySmartRecipeOrdering(items = []) {
            const usage = getRecipeUsageMap();
            return (items || []).map((item, index) => {
                const id = String(item?.recipe?.id || '');
                const smart = usage[id] || {};
                return {
                    item,
                    index,
                    smartScore: (Number(smart.count) || 0) * 1000000000000 + (Number(smart.lastUsedAt) || 0)
                };
            }).sort((a, b) => b.smartScore - a.smartScore || a.index - b.index).map(entry => entry.item);
        }

        function getMostUsedRecipeId() {
            const usage = getRecipeUsageMap();
            return Object.keys(usage).sort((a, b) =>
                (Number(usage[b]?.count) || 0) - (Number(usage[a]?.count) || 0) ||
                (Number(usage[b]?.lastUsedAt) || 0) - (Number(usage[a]?.lastUsedAt) || 0)
            )[0] || '';
        }

        function getSmartRecentProductId() {
            const usage = getProductUsageMap();
            const products = getSortedManualProducts();
            const productIds = new Set(products.map(product => String(product.id)));
            const recentUsedId = Object.keys(usage).sort((a, b) =>
                (Number(usage[b]?.lastUsedAt) || 0) - (Number(usage[a]?.lastUsedAt) || 0)
            ).find(id => productIds.has(String(id)));
            if (recentUsedId) return recentUsedId;
            return products[0]?.id || '';
        }

        function getSmartFrequentProductId() {
            const usage = getProductUsageMap();
            const products = loadManualProducts().filter(isValidManualProduct);
            return products.sort((a, b) =>
                (Number(usage[String(b.id)]?.count) || 0) - (Number(usage[String(a.id)]?.count) || 0) ||
        ...
        function openAddMealAction(event) {
            ...
            openDiaryFoodPickerScreen(getCurrentAddMealType(), event, { activeTab: 'products' });
        }
        ...
        function chooseRecipeAddFlow() {
            closeAddMealChoice();
            openDiaryFoodPickerScreen(getCurrentAddMealType(), null, { activeTab: 'my-recipes' });
        }
        ...
        async function openBarcodeCameraPlaceholder(event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            ...
            if (!window.isSecureContext && location.protocol !== 'file:') {
                setBarcodeCameraStatus('Камера доступна только в безопасном соединении. Откройте Mini App через Telegram/HTTPS или используйте ввод штрихкода вручную.');
                console.warn('[barcode camera] getUserMedia requires a secure context');
                return;
            }
        ...
        function openBarcodeMealModal(options = {}) {
            ...
            if (options?.startCamera) {
                setTimeout(() => openBarcodeCameraPlaceholder(), 120);
            } else {
        ...
        function renderBarcodeProductCard(product) {
            ...
            result.innerHTML = '<section class="barcode-product-card">' +
                ...
                '<div class="barcode-macro-line">' + Math.round(product.kcal100) + ' ккал • Б ' + Math.round(Number(product.protein100) || 0) + ' • Ж ' + Math.round(Number(product.fat100) || 0) + ' • У ' + Math.round(Number(product.carbs100) || 0) + '</div>' +
        ...
        function setProductSearchMealError(message = '') {
            const el = document.getElementById('product-search-meal-error');
            if (!el) return;
            el.textContent = message;
            el.classList.toggle('active', Boolean(message));
        }

        function productSearchMealTypeStorageKey() {
            return 'product_search_meal_type_' + appUserId;
        }

        function getProductSearchMealType() {
            return getValidMealType(localStorage.getItem(productSearchMealTypeStorageKey()) || '', getCurrentAddMealType());
        }

        function saveProductSearchMealType(mealType) {
            localStorage.setItem(productSearchMealTypeStorageKey(), getValidMealType(mealType, getCurrentAddMealType()));
        }

        function mapSearchProductToMealDraft(product) {
            const draft = mapSearchProduct(product, product?.source || 'search');
            ...
            return {
                ...
                name: draft.name,
                brand: product.brand || '',
                imageUrl: safeImageUrl(product.imageUrl || product.image_url || product.image || ''),
                kcal100: Number(draft.caloriesPer100) || 0,
                protein100: Number(draft.proteinPer100) || 0,
                fat100: Number(draft.fatPer100) || 0,
                carbs100: Number(draft.carbsPer100) || 0,
                source: product.source || 'search',
                sourceLabel: product.sourceLabel || draft.sourceLabel || '',
                hasKbju: true
            };
        }

        function getProductSearchMealNumber(id) {
            const raw = document.getElementById(id)?.value;
            if (raw === '' || raw === null || raw === undefined) return 0;
            return Number(String(raw).replace(',', '.'));
        }

        function calculateProductSearchMealTotals() {
            const product = productSearchMealDraft;
            const grams = getProductSearchMealNumber('product-search-meal-grams');
            const ratio = grams > 0 ? grams / 100 : 0;
            return {
                grams,
                kcal: (Number(product?.kcal100) || 0) * ratio,
        ...
        function updateProductSearchMealTotals() {
            const totals = calculateProductSearchMealTotals();
            setText('product-search-total-title', 'Для ' + formatProductSearchMacroValue(totals.grams) + ' г');
            setText('product-search-total-kcal', String(Math.round(Math.max(0, totals.kcal || 0))));
            setText('product-search-total-protein', (Math.max(0, totals.protein || 0)).toFixed(1) + ' г');
            setText('product-search-total-fat', (Math.max(0, totals.fat || 0)).toFixed(1) + ' г');
            setText('product-search-total-carbs', (Math.max(0, totals.carbs || 0)).toFixed(1) + ' г');
            document.querySelectorAll('.product-search-quick-grams button[data-grams]').forEach(btn => {
                const grams = Number(btn.dataset.grams) || 0;
                btn.classList.toggle('is-active', Math.abs(grams - totals.grams) < 0.01);
            });
        }

        function setProductSearchMealGrams(grams) {
            const input = document.getElementById('product-search-meal-grams');
            if (input) input.value = String(Math.max(1, Number(grams) || 100));
            updateProductSearchMealTotals();
        }

        function formatProductSearchMacroValue(value) {
            const number = Number(value) || 0;
            return Number.isInteger(number) ? String(number) : number.toFixed(1);
        }

        function renderProductSearchMealCard(product) {
            const result = document.getElementById('product-search-meal-result');
            if (!result) return;
            const mealType = getProductSearchMealType();
            const title = product.brand ? product.name + ' · ' + product.brand : product.name;
            result.innerHTML = '<section class="barcode-product-card product-search-product-card">' +
                '<div class="product-search-product-head">' +
                    '<div class="product-search-product-copy"><div class="barcode-product-title">' + escapeHTML(title) + '</div></div>' +
                '</div>' +
                '<div class="barcode-meal-grid">' +
                    '<label class="barcode-meal-field"><span>Приём пищи</span><select id="product-search-meal-type" onchange="saveProductSearchMealType(this.value)"><option value="Завтрак">Завтрак</option><option value="Обед">Обед</option><option value="Ужин">Ужин</option><option value="Перекус">Перекус</option></select></label>' +
                    '<label class="barcode-meal-field"><span>Вес порции, г</span><input id="product-search-meal-grams" type="number" inputmode="decimal" min="1" step="1" value="100" oninput="updateProductSearchMealTotals()"></label>' +
                '</div>' +
                '<div class="product-search-quick-grams" aria-label="Быстрый выбор веса">' +
                    '<button type="button" data-grams="50" onclick="setProductSearchMealGrams(50)">50 г</button>' +
                    '<button type="button" data-grams="100" onclick="setProductSearchMealGrams(100)">100 г</button>' +
                    '<button type="button" data-grams="150" onclick="setProductSearchMealGrams(150)">150 г</button>' +
                    '<button type="button" data-grams="200" onclick="setProductSearchMealGrams(200)">200 г</button>' +
                '</div>' +
                '<div class="barcode-total-card"><div class="barcode-total-title" id="product-search-total-title">Для 100 г</div><div class="barcode-total-grid">' +
                    '<div><b id="product-search-total-kcal">0</b><span>ккал</span></div>' +
                    '<div><b id="product-search-total-protein">0.0 г</b><span>белки</span></div>' +
                    '<div><b id="product-search-total-fat">0.0 г</b><span>жиры</span></div>' +
                    '<div><b id="product-search-total-carbs">0.0 г</b><span>углеводы</span></div>' +
                '</div></div>' +
                '<button class="barcode-add-btn product-search-add-btn" id="product-search-meal-add-btn" type="button" onclick="addProductSearchMealToDiary()">Добавить в дневник</button>' +
                '</section>';
            const select = document.getElementById('product-search-meal-type');
            if (select) select.value = mealType;
            updateProductSearchMealTotals();
        }

        function openProductSearchMealModal(product) {
            const draft = mapSearchProductToMealDraft(product);
            if (!draft) return showToast('Продукт не найден');
            productSearchMealDraft = draft;
            setProductSearchMealError('');
            renderProductSearchMealCard(draft);
            setLockedLayer('product-search-meal', document.getElementById('product-search-meal-modal'), true);
            setTimeout(() => document.getElementById('product-search-meal-grams')?.focus?.(), 120);
        }

        function closeProductSearchMealModal() {
            productSearchMealDraft = null;
            setProductSearchMealError('');
            setLockedLayer('product-search-meal', document.getElementById('product-search-meal-modal'), false);
            const result = document.getElementById('product-search-meal-result');
            if (result) result.innerHTML = '';
        }

        async function addProductSearchMealToDiary() {
            if (isAddingMeal) return;
            const product = productSearchMealDraft;
            if (!product) return setProductSearchMealError('Сначала выберите продукт.');
            const mealType = document.getElementById('product-search-meal-type')?.value || getCurrentAddMealType() || 'Перекус';
            const totals = calculateProductSearchMealTotals();
            if (!Number.isFinite(totals.grams) || totals.grams <= 0) return setProductSearchMealError('Вес порции должен быть больше 0 г.');
            saveProductSearchMealType(mealType);
            const btn = document.getElementById('product-search-meal-add-btn');
            isAddingMeal = true;
            if (btn) btn.disabled = true;
            setProductSearchMealError('');
            const createdAt = selectedDateTimeISO();
            ...
            try {
                ...
                console.error('Ошибка добавления продукта из поиска:', error);
                if (isTelegramMiniApp) {
                    ...
                    setProductSearchMealError('Не удалось добавить продукт: ' + error.message);
                }
            } finally {
                isAddingMeal = false;
                if (btn) btn.disabled = false;
        ...
        function setMultiProductMealError(message = '') {
            const el = document.getElementById('multi-product-meal-error');
            if (!el) return;
            el.textContent = message;
            el.classList.toggle('active', Boolean(message));
        }

        function getMultiProductItemTotals(item) {
            const grams = Math.max(0, Number(item?.grams) || 0);
            const product = item?.product || {};
            const ratio = grams / 100;
            return {
                grams,
                kcal: (Number(product.caloriesPer100) || 0) * ratio,
                protein: (Number(product.proteinPer100) || 0) * ratio,
                fat: (Number(product.fatPer100) || 0) * ratio,
                carbs: (Number(product.carbsPer100) || 0) * ratio
            };
        }

        function getMultiProductMealTotals() {
            return (multiProductMealDraft?.items || []).reduce((sum, item) => {
                const totals = getMultiProductItemTotals(item);
        ...
        function updateMultiProductMealTotals() {
            if (!multiProductMealDraft) return;
            multiProductMealDraft.items.forEach((item, index) => {
                const totals = getMultiProductItemTotals(item);
                setText('multi-product-item-kbju-line-' + index, Math.round(Math.max(0, totals.kcal)) + ' ккал • Б ' + Math.max(0, totals.protein).toFixed(1) + ' • Ж ' + Math.max(0, totals.fat).toFixed(1) + ' • У ' + Math.max(0, totals.carbs).toFixed(1));
            });
            const totals = getMultiProductMealTotals();
            setText('multi-product-total-kcal', String(Math.round(Math.max(0, totals.kcal))));
            setText('multi-product-total-protein', Math.max(0, totals.protein).toFixed(1) + ' г');
            setText('multi-product-total-fat', Math.max(0, totals.fat).toFixed(1) + ' г');
            setText('multi-product-total-carbs', Math.max(0, totals.carbs).toFixed(1) + ' г');
        }

        function updateMultiProductMealItem(index, value) {
            if (!multiProductMealDraft?.items?.[index]) return;
            const grams = Number(String(value || '').replace(',', '.'));
            multiProductMealDraft.items[index].grams = Number.isFinite(grams) ? grams : 0;
            updateMultiProductMealTotals();
        }

        function renderMultiProductMealModal() {
            const result = document.getElementById('multi-product-meal-result');
            if (!result || !multiProductMealDraft) return;
            const mealType = getValidMealType(multiProductMealDraft.mealType, getDiaryMealType());
            const itemsHtml = multiProductMealDraft.items.map((item, index) => {
                const product = item.product || {};
                return '<article class="multi-product-item">' +
                    '<div class="multi-product-item-head">' +
                        '<b>' + escapeHTML(product.name || 'Продукт') + '</b>' +
                        '<label><span>Вес, г</span><input id="multi-product-grams-' + index + '" type="number" inputmode="decimal" min="1" step="1" value="' + escapeAttr(String(Math.max(1, Number(item.grams) || 100))) + '" oninput="updateMultiProductMealItem(' + index + ', this.value)"></label>' +
                    '</div>' +
                    '<div class="multi-product-item-kbju">' +
                        '<span id="multi-product-item-kbju-line-' + index + '">0 ккал • Б 0.0 • Ж 0.0 • У 0.0</span>' +
                    '</div>' +
                '</article>';
            }).join('');
            result.innerHTML = '<section class="multi-product-card">' +
                '<div class="multi-product-meal-type">Приём пищи: <b>' + escapeHTML(mealType) + '</b></div>' +
                '<div class="multi-product-list">' + itemsHtml + '</div>' +
                '<div class="barcode-total-card multi-product-total"><div class="barcode-total-title">Итого</div><div class="barcode-total-grid">' +
                    '<div><b id="multi-product-total-kcal">0</b><span>ккал</span></div>' +
                    '<div><b id="multi-product-total-protein">0.0 г</b><span>белки</span></div>' +
                    '<div><b id="multi-product-total-fat">0.0 г</b><span>жиры</span></div>' +
                    '<div><b id="multi-product-total-carbs">0.0 г</b><span>углеводы</span></div>' +
                '</div></div>' +
                '<button class="barcode-add-btn multi-product-add-btn" id="multi-product-meal-add-btn" type="button" onclick="addMultiProductMealToDiary()">Добавить в дневник</button>' +
            '</section>';
            updateMultiProductMealTotals();
        }

        function openMultiProductMealModal() {
            const items = Array.from(diaryMealSelectedProducts.values()).map(entry => {
                const product = entry.product;
                const grams = Math.max(1, Number(product.defaultGrams || product.grams || product.servingGrams) || 100);
                return { key: entry.key, product, grams };
            });
            if (!items.length) return;
            multiProductMealDraft = {
                mealType: getDiaryMealType(),
                items
            };
            setMultiProductMealError('');
            renderMultiProductMealModal();
            setLockedLayer('multi-product-meal', document.getElementById('multi-product-meal-modal'), true);
        }

        function setMultiProductDraftFromSelection() {
            const items = Array.from(diaryMealSelectedProducts.values()).map(entry => {
                const product = entry.product;
                const grams = Math.max(1, Number(product.defaultGrams || product.grams || product.servingGrams) || 100);
                return { key: entry.key, product, grams };
        ...
        async function addSelectedDiaryMealProducts(event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            if (isAddingMeal) return;
            if (!setMultiProductDraftFromSelection()) return showToast('Выберите продукты');
            showToast('Добавляю продукты...');
            await addMultiProductMealToDiary();
        }

        function closeMultiProductMealModal() {
            multiProductMealDraft = null;
            setMultiProductMealError('');
            setLockedLayer('multi-product-meal', document.getElementById('multi-product-meal-modal'), false);
            const result = document.getElementById('multi-product-meal-result');
            if (result) result.innerHTML = '';
        }

        async function addMultiProductMealToDiary() {
            if (isAddingMeal) return;
            const draft = multiProductMealDraft;
            if (!draft?.items?.length) return setMultiProductMealError('Выберите хотя бы один продукт.');
            const mealType = getValidMealType(draft.mealType, getDiaryMealType());
            if (draft.items.some(item => !Number.isFinite(Number(item.grams)) || Number(item.grams) <= 0)) {
                return setMultiProductMealError('Вес каждой позиции должен быть больше 0 г.');
            }
            const payloads = draft.items.map(item => {
                const grams = Number(item.grams);
                const payload = buildDiaryManualProductPayload({ ...item.product, grams }, mealType);
                payload.barcode = item.product.barcode || '';
                payload.brand = item.product.brand || '';
                return payload;
            });
            const btn = document.getElementById('multi-product-meal-add-btn');
            isAddingMeal = true;
            if (btn) btn.disabled = true;
            setMultiProductMealError('');
            try {
                for (let i = 0; i < payloads.length; i += 1) {
                    try {
                        await callServer('addMeal', payloads[i]);
                    } catch (error) {
                        if (!isTelegramMiniApp) throw error;
                        addManualLocalMeal(payloads[i]);
                    }
                    const product = draft.items[i].product;
                    upsertManualProduct(product.name, {
                        kcal100: product.caloriesPer100,
                        protein100: product.proteinPer100,
                        fat100: product.fatPer100,
                        carbs100: product.carbsPer100
                    }, { barcode: product.barcode });
                }
                closeMultiProductMealModal();
                diaryMealSelectedProducts.clear();
                renderDiaryMealSelectionBar();
                closeDiaryMealScreen();
                await refreshAllData();
                document.getElementById('history-list')?.classList.add('success-flash');
                setTimeout(() => document.getElementById('history-list')?.classList.remove('success-flash'), 700);
                showToast('Добавлено продуктов: ' + payloads.length);
            } catch (error) {
                console.error('Ошибка добавления нескольких продуктов:', error);
                setMultiProductMealError('Не удалось добавить продукты: ' + error.message);
            } finally {
                isAddingMeal = false;
                if (btn) btn.disabled = false;
            }
        ...
        function formatMyRecipeMacroLine(nutrition) {
            ...
            return Math.round(Number(n.calories) || 0) + ' ккал • Б ' +
                Math.round(Number(n.protein) || 0) + ' • Ж ' +
                Math.round(Number(n.fat) || 0) + ' • У ' +
        ...
        function renderMyRecipeGrid(recipes = [], options = {}) {
        ...
        function formatMyRecipeDetailMacroLine(nutrition, withGramUnits = false) {
            ...
            return Math.round(Number(nutrition?.calories) || Number(nutrition?.kcal) || 0) + ' ккал • Б ' +
                Math.round(Number(nutrition?.protein) || 0) + unit + ' • Ж ' +
                Math.round(Number(nutrition?.fat) || 0) + unit + ' • У ' +
                Math.round(Number(nutrition?.carbs) || 0) + unit;
        }
        ...
        async function quickAddMyRecipeToDiary(recipeId, event = null) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const recipe = loadManualRecipes().find(item => String(item.id) === String(recipeId));
            if (!recipe) return showToast('Рецепт не найден');
            const resolved = resolveMyRecipeDetailsNutrition(recipe);
            const grams = getMyRecipeDetailsInitialGrams(recipe, resolved);
            const mealType = getValidMealType(currentDiaryMealType || recipe.category, 'Обед');
            showToast('Добавляю в ' + mealType + '...');
        ...
        function openMyRecipeAddModal(recipeId, options = {}) {
            ...
            if (mealType) {
                mealType.value = isDiaryMealScreenOpen() && !currentDiaryMealType ? '' : currentDiaryMealType ? getValidMealType(currentDiaryMealType, 'Обед') : getValidMealType(selectedMyRecipeForDiary?.category, 'Обед');
        ...
        function setMyRecipePhotoPreview(imageUrl = '') {
            ...
            setText('my-recipe-photo-title', myRecipeImageData ? 'Заменить фото' : 'Добавить фото');
            setText('my-recipe-photo-subtitle', myRecipeImageData ? 'Выбрано изображение рецепта' : 'Фото рецепта сделает карточку понятнее');
        }
        ...
        function handleMyRecipePhotoChange(event) {
            ...
            if (!String(file.type || '').startsWith('image/')) {
                setMyRecipeFormError('Выберите файл изображения.');
                return;
            }
            ...
            reader.onerror = () => setMyRecipeFormError('Не удалось загрузить фото. Попробуйте другое изображение.');
        ...
        function getMyRecipeStepMeta(step = myRecipeCreateStep) {
            if (step === 2) return { kicker: 'Шаг 2 из 2', title: 'Рецепт' };
            return { kicker: 'Шаг 1 из 2', title: 'Ингредиенты' };
        }
        ...
        function setMyRecipeCreateStep(step, options = {}) {
            ...
            const primary = document.getElementById('my-recipe-primary-action');
            if (primary) primary.textContent = myRecipeCreateStep === 2 ? (editingMyRecipeId ? 'Сохранить изменения' : 'Сохранить рецепт') : 'Далее';
            if (primary && myRecipeCreateStep === 2) {
                primary.innerHTML = '<span class="my-recipe-primary-icon" aria-hidden="true">✓</span><span>' + (editingMyRecipeId ? 'Сохранить изменения' : 'Сохранить рецепт') + '</span>';
            }
        ...
        function toggleFavorite(event, recipeId) {
            event.stopPropagation();
            console.log('TOGGLE_RECIPE_FAVORITE', recipeId);
        ...
        function syncRecipeDetailsFavoriteButton(recipeId) {
            const favs = (JSON.parse(localStorage.getItem('fav_recipes_' + appUserId)) || []).map(String);
            const active = favs.includes(String(recipeId));
            ['detail-favorite-btn', 'recipe-view-favorite-btn'].forEach(id => {
        ...
        function toggleRecipeDetailsFavorite(event) {
            event?.stopPropagation?.();
            if (!recipeDetailsCurrentId) return;
            toggleFavorite({ stopPropagation() {} }, recipeDetailsCurrentId);
            syncRecipeDetailsFavoriteButton(recipeDetailsCurrentId);
        }

        function formatRecipeCompactMacroValue(value) {
            const number = Number(value) || 0;
            return Number.isInteger(number) ? String(number) : number.toFixed(1);
        }

        function formatRecipeCompactKbjuLine(label, kcal, protein, fat, carbs) {
            return label + ': ' + Math.round(Number(kcal) || 0) + ' ккал • Б ' +
                formatRecipeCompactMacroValue(protein) + ' • Ж ' +
                formatRecipeCompactMacroValue(fat) + ' • У ' +
                formatRecipeCompactMacroValue(carbs);
        }

        function formatRecipeAddKbjuLine(grams, kcal, protein, fat, carbs, label = '') {
            const prefix = label || formatRecipeCompactMacroValue(grams) + ' г';
            return prefix + ' • ' + Math.round(Number(kcal) || 0) + ' ккал • Б ' +
                formatRecipeCompactMacroValue(protein) + ' • Ж ' +
                formatRecipeCompactMacroValue(fat) + ' • У ' +
                formatRecipeCompactMacroValue(carbs);
        }

        function getRecipePortionWeight(recipe, total = null) {
            const resolvedTotal = total || getRecipePortionNutrition(recipe, getRecipeWorkingIngredients(recipe));
            const servings = Math.max(1, Number(recipe?.servings) || 1);
            return Math.max(1, Math.round((Number(resolvedTotal?.grams) || 100) / servings));
        }

        function getRecipeAddQuickOptions(recipe, total) {
            const servings = Number(recipe?.servings) || 0;
            const portion = getRecipePortionWeight(recipe, total);
            if (servings > 0 && (Number(total?.grams) || 0) > 0) {
                return [
                    { value: portion * 0.5, label: '½ порции' },
                    { value: portion, label: '1 порция' },
                    { value: portion * 1.5, label: '1½ порции' },
                    { value: portion * 2, label: '2 порции' }
                ];
            }
            return [100, 150, 200, 300].map(value => ({ value, label: value + ' г' }));
        }

        function setRecipePortionQuickGrams(value, label = '') {
            if (!recipePortionDraft) return;
            recipePortionDraft.eatenGrams = Math.max(1, Number(value) || 100);
            recipePortionDraft.eatenLabel = label || '';
        ...
        function openRecipePortionEditor(mode, recipeId, context = {}) {
            ...
            const defaultDiaryGrams = isDiaryMode ? getRecipePortionWeight(recipe, total) : Math.round(total.grams || 100);
            const defaultEatenGrams = Math.max(1, Number(context.eatenGrams) || defaultDiaryGrams);
            recipePortionDraft = {
                mode,
                recipeId: String(recipe.id),
                context,
                ingredients,
                eatenGrams: defaultEatenGrams,
                eatenLabel: context.eatenLabel || (isDiaryMode && !Number(context.eatenGrams) && Number(recipe?.servings) > 0 ? '1 порция' : ''),
                mealType: context.mealType || (isDiaryMode ? getValidMealType(currentDiaryMealType || currentMealFilter || recipe.category, 'Обед') : recipe.category || currentMealFilter || 'Обед')
            };
            document.getElementById('recipe-portion-overlay')?.classList.toggle('is-diary-add', isDiaryMode);
            document.getElementById('portion-title').textContent = isDiaryMode ? 'Добавить в дневник' : recipe.title;
            const subtitle = document.getElementById('portion-editor-subtitle');
            if (subtitle) subtitle.textContent = isDiaryMode ? (recipe.title || 'Выберите вес порции и приём пищи.') : recipe.description || 'Настрой рецепт и граммовки ингредиентов.';
        ...
        function renderRecipePortionTotals() {
            ...
            if (recipePortionDraft.mode === 'diary') {
                document.getElementById('portion-total').innerHTML =
                    '<div class="portion-add-summary-line">' +
                    escapeHTML(formatRecipeAddKbjuLine(eatenGrams, total.kcal * eatenRatio, total.protein * eatenRatio, total.fat * eatenRatio, total.carbs * eatenRatio, recipePortionDraft.eatenLabel || '')) +
                    '</div>';
            } else {
                document.getElementById('portion-total').innerHTML =
                    '<div class="portion-kbju-card"><div><div class="portion-kbju-title">КБЖУ / 100 г</div>' +
                    '<div class="portion-kbju-line">' + Math.round(total.kcal * per100Ratio) + ' ккал • Б ' + Math.round(total.protein * per100Ratio) + ' • Ж ' + Math.round(total.fat * per100Ratio) + ' • У ' + Math.round(total.carbs * per100Ratio) + '</div></div>' +
            ...
            const quick = document.getElementById('portion-quick-grams');
            if (quick) {
                const recipe = getRecipeById(recipePortionDraft.recipeId);
                const quickOptions = getRecipeAddQuickOptions(recipe, total);
                quick.hidden = recipePortionDraft.mode !== 'diary';
                quick.innerHTML = recipePortionDraft.mode === 'diary' ? quickOptions.map(option => {
                    const active = Math.abs(option.value - eatenGrams) < 0.01 && (!recipePortionDraft.eatenLabel || recipePortionDraft.eatenLabel === option.label);
                    return '<button class="' + (active ? 'active' : '') + '" type="button" onclick="setRecipePortionQuickGrams(' + option.value + ', ' + escapeAttr(JSON.stringify(option.label)) + ')">' + escapeHTML(option.label) + '</button>';
                }).join('') : '';
            }
        }
        ...
        function confirmRecipePortion() {
            ...
            const mealType = getValidMealType(recipePortionDraft.mealType, mode === 'diary' ? '' : recipe.category || 'Перекус');
            if (mode === 'diary' && !mealType) return showToast('Выберите приём пищи.');
            const eatenRatio = total.grams > 0 ? eatenGrams / total.grams : 0;
        ...
        function getGoalBadgePriority() {
            ...
            const filtered = applySmartRecipeOrdering(sortRecipeItems(getEnrichedRecipes()
        ...
        function openRecipesScreen() {
            ...
            if (!overlay) return;
            const v2Root = document.getElementById('recipes-v2-root');
            const legacyUi = document.getElementById('recipes-legacy-ui');
            v2Root?.toggleAttribute('hidden', !USE_RECIPES_V2);
            legacyUi?.toggleAttribute('hidden', USE_RECIPES_V2);
            overlay.classList.toggle('is-recipes-v2', USE_RECIPES_V2);
        ...
        function setRecipesV2Tab(tab) {
            recipesV2ActiveTab = ['catalog', 'my', 'favorites'].includes(tab) ? tab : 'catalog';
            renderRecipesScreen();
        }

        function getRecipesV2CatalogItems(favs) {
            const q = recipeSearchQuery;
            const searchableIds = new Set(searchRecipes(recipesData, q).map(recipe => String(recipe.id)));
            return applySmartRecipeOrdering(sortRecipeItems(getEnrichedRecipes()
        ...
        function renderRecipesV2Screen() {
            const root = document.getElementById('recipes-v2-root');
            if (!root || !window.RecipesV2) return;
            const favs = (JSON.parse(localStorage.getItem('fav_recipes_' + appUserId)) || []).map(String);
            const items = getRecipesV2CatalogItems(favs);
            const favoriteItems = items.filter(item => favs.includes(String(item.recipe.id)));
            const metaItems = recipesV2ActiveTab === 'favorites' ? favoriteItems : items;
            window.RecipesV2.render({
                activeTab: recipesV2ActiveTab,
                subtitle: recipesV2ActiveTab === 'my' ? 'Собственные рецепты и создание' : 'Поиск, фильтры и избранное',
                search: recipeSearchQuery,
                sort: recipeSortMode,
                mealFilter: screenMealFilter,
                dietFilter: screenDietFilter,
                items,
                favoriteItems,
                favs,
                myRecipesHtml: getRecipesV2MyRecipesHtml(),
                meta: metaItems.length + ' ' + (metaItems.length === 1 ? 'рецепт' : 'рецептов') + ' · ' + screenMealFilter + ' · ' + screenDietFilter
            });
        }

        function renderRecipesScreen() {
            if (USE_RECIPES_V2) {
                renderRecipesV2Screen();
                return;
            }
            ...
            grid.classList.add('recipe-grid', 'recipe-retail-list');
            ...
            grid.innerHTML = renderRecipeGrid(filtered, favs);
        }
        ...
        document.addEventListener('click', function(event) {
            const addButton = event.target.closest('[data-action="add-recipe"], .recipe-add-btn');
            if (!addButton) return;
            event.preventDefault();
            event.stopPropagation();
        ...
        document.addEventListener('click', function(event) {
            ...
            if (actionTarget && actionTarget.dataset.action !== 'open-recipe') return;
            const card = event.target.closest('[data-action="open-recipe"][data-recipe-open-id], [data-recipe-open-id]');
            if (!card) return;
            const page = document.getElementById('recipe-view-page');
            if (page && !page.hasAttribute('hidden')) return;
            openRecipeDetails(card.dataset.recipeOpenId);
        });

        function addRecipeToDiary(recipeId, portionGrams = null) {
            console.log('OPEN_ADD_RECIPE_MODAL', recipeId);
            const recipe = getRecipeById(recipeId);
            if (!recipe) return showToast('Рецепт не найден');
            return openRecipePortionEditor('diary', recipe.id, { eatenGrams: portionGrams });
        }

        function quickAddRecipeToDiary(recipeId, event = null, action = null) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const recipe = getRecipeById(recipeId);
            if (!recipe) return showToast('Рецепт не найден');
            ...
            const total = getRecipePortionNutrition(recipe, ingredients);
            const quickAction = action || getDefaultRecipeQuickAction(recipe.id);
            const baseGrams = getRecipePortionWeight(recipe, total);
            const eatenGrams = quickAction === 'half' ? Math.max(1, Math.round(baseGrams / 2)) : baseGrams;
            const eatenRatio = total.grams > 0 ? eatenGrams / total.grams : 1;
            const mealType = quickAction === 'last-meal'
                ? getLastSmartMealType(recipe.category || 'Обед')
                : getSmartDefaultMealType(recipe.category || 'Обед');
            showToast('Добавляю в ' + mealType + '...');
            return openMealModal(
                recipe.id,
                total.kcal * eatenRatio,
                total.protein * eatenRatio,
                total.fat * eatenRatio,
                total.carbs * eatenRatio,
                scalePortionIngredients(ingredients, eatenRatio),
                mealType,
                { source: 'quick-recipe', action: quickAction, eatenGrams }
            );
        }
        ...
        function formatMacroLine(nutrition, withGramUnits = false) {
            const unit = withGramUnits ? ' г' : '';
            return Math.round(Number(nutrition?.kcal) || 0) + ' ккал • Б ' + Math.round(Number(nutrition?.protein) || 0) + unit + ' • Ж ' + Math.round(Number(nutrition?.fat) || 0) + unit + ' • У ' + Math.round(Number(nutrition?.carbs) || 0) + unit;
        }
        ...
        function updateDiaryMealFooterVisibility() {
            ...
            const rawProgress = diaryMealSourceTab === 'library' ? Math.min(1, Math.max(0, content.scrollTop / 96)) : 0;
            const progress = Math.pow(rawProgress, 0.72);
            const search = document.getElementById('diary-meal-search-input');
            const keepFooterVisible = diaryMealSourceTab === 'library' || diaryMealSelectedProducts.size > 0 || Boolean(document.activeElement?.closest?.('.diary-meal-footer')) || Boolean(String(search?.value || '').trim());
            const footerProgress = keepFooterVisible ? 0 : progress;
            screen.style.setProperty('--diary-footer-progress', progress.toFixed(3));
            screen.style.setProperty('--diary-footer-control-progress', footerProgress.toFixed(3));
            screen.style.setProperty('--diary-footer-opacity', (1 - footerProgress).toFixed(3));
            screen.style.setProperty('--diary-glow-opacity', (1 - progress).toFixed(3));
            screen.classList.toggle('is-glow-hidden', progress >= 0.98);
            screen.classList.toggle('is-footer-hidden', footerProgress >= 0.98);
        }

        function openDiaryMealScreen(mealType, event, options = {}) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const targetMealType = mealType ? getValidMealType(mealType, currentDiaryMealType || 'Завтрак') : getCurrentAddMealType();
            const targetTab = options.activeTab === 'recipes' ? 'my-recipes' : 'products';
            openDiaryFoodPickerScreen(targetMealType, null, { activeTab: targetTab });
            return;
            currentDiaryMealType = mealType ? getValidMealType(mealType, currentDiaryMealType || 'Завтрак') : '';
            ...
            renderDiaryMealContent();
            const screen = document.getElementById('diary-meal-screen');
            screen?.classList.remove('is-footer-hidden', 'is-glow-hidden');
            screen?.style.setProperty('--diary-footer-progress', '0');
            screen?.style.setProperty('--diary-footer-control-progress', '0');
            screen?.style.setProperty('--diary-footer-opacity', '1');
            screen?.style.setProperty('--diary-glow-opacity', '1');
            screen?.removeAttribute('hidden');
            screen?.setAttribute('aria-hidden', 'false');
            setLockedLayer('diary-meal', screen, true);
            if (options.focusSearch) setTimeout(() => search?.focus?.(), 120);
        ...
        const DIARY_FOOD_PICKER_TABS = ['quick', 'products', 'my-recipes', 'cooking'];
        let diaryFoodPickerBulkPressTimer = 0;
        let diaryFoodPickerBulkPressHandled = false;
        const diaryFoodPickerState = {
            ...
            productEditId: null,
            productCreateDraft: { name: '', kcal: '', protein: '', fat: '', carbs: '' },
            productCreateError: '',
            quickCreateMode: false,
            quickEditId: null,
            quickCreateDraft: { name: '', grams: '', kcal: '', protein: '', fat: '', carbs: '' },
            quickCreateError: '',
            selectedProducts: {},
            bulkSelectMode: null,
        ...
        function getDiaryFoodPickerTabIndex(tabName) {
            ...
            return index >= 0 ? index : DIARY_FOOD_PICKER_TABS.indexOf('products');
        }

        function getDiaryFoodPickerTabTitle(tabName) {
            if (tabName === 'quick') return 'Быстрый ввод';
            if (tabName === 'my-recipes') return 'Мои рецепты';
            if (tabName === 'cooking') return 'Кулинария';
            return 'Продукты';
        }

        function getDiaryFoodPickerBulkMap(type) {
            if (!diaryFoodPickerState.bulkSelected) diaryFoodPickerState.bulkSelected = { quick: {}, products: {}, recipes: {} };
            if (!diaryFoodPickerState.bulkSelected[type]) diaryFoodPickerState.bulkSelected[type] = {};
            return diaryFoodPickerState.bulkSelected[type];
        }

        function isDiaryFoodPickerBulkMode(type) {
            return type ? diaryFoodPickerState.bulkSelectMode === type : Boolean(diaryFoodPickerState.bulkSelectMode);
        }

        function isDiaryFoodPickerBulkSelected(type, id) {
            return Boolean(getDiaryFoodPickerBulkMap(type)[String(id || '')]);
        }

        function getDiaryFoodPickerBulkSelectedIds(type = diaryFoodPickerState.bulkSelectMode) {
            if (!type) return [];
            return Object.keys(getDiaryFoodPickerBulkMap(type));
        }

        function resetDiaryFoodPickerBulkSelection() {
            clearTimeout(diaryFoodPickerBulkPressTimer);
            diaryFoodPickerBulkPressTimer = 0;
            diaryFoodPickerBulkPressHandled = false;
            diaryFoodPickerState.bulkSelectMode = null;
            diaryFoodPickerState.bulkSelected = { quick: {}, products: {}, recipes: {} };
        }

        function cancelDiaryFoodPickerBulkPress() {
            clearTimeout(diaryFoodPickerBulkPressTimer);
            diaryFoodPickerBulkPressTimer = 0;
        }

        function consumeDiaryFoodPickerBulkPress() {
            const handled = diaryFoodPickerBulkPressHandled;
            diaryFoodPickerBulkPressHandled = false;
            cancelDiaryFoodPickerBulkPress();
            return handled;
        }

        function canDiaryFoodPickerBulkSelect(type, id) {
            if (type !== 'products') return Boolean(id);
            const product = getDiaryFoodPickerProducts().find(item => String(item.id) === String(id));
            return product?.source === 'manual';
        }

        function startDiaryFoodPickerBulkPress(type, id, event) {
            if (!['quick', 'products', 'recipes'].includes(type)) return;
            const key = String(id || '');
            if (!key) return;
            cancelDiaryFoodPickerBulkPress();
            diaryFoodPickerBulkPressHandled = false;
            diaryFoodPickerBulkPressTimer = setTimeout(() => {
                if (!canDiaryFoodPickerBulkSelect(type, key)) {
                    showToast(type === 'products' ? 'Можно удалить только свои продукты' : 'Объект недоступен');
                    return;
                }
                diaryFoodPickerBulkPressHandled = true;
                diaryFoodPickerState.bulkSelectMode = type;
                diaryFoodPickerState.bulkSelected = { quick: {}, products: {}, recipes: {} };
                getDiaryFoodPickerBulkMap(type)[key] = true;
        ...
        function toggleDiaryFoodPickerBulkSelection(type, id, event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const key = String(id || '');
            if (!key || !isDiaryFoodPickerBulkMode(type)) return;
            if (!canDiaryFoodPickerBulkSelect(type, key)) return showToast('Можно удалить только свои продукты');
            const selected = getDiaryFoodPickerBulkMap(type);
            if (selected[key]) {
                delete selected[key];
            } else {
                selected[key] = true;
        ...
        function clearDiaryFoodPickerBulkSelection(event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            resetDiaryFoodPickerBulkSelection();
            renderDiaryFoodPickerScreen();
        ...
        function renderDiaryFoodPickerBulkBar(type) {
            if (!isDiaryFoodPickerBulkMode(type)) return '';
            const count = getDiaryFoodPickerBulkSelectedIds(type).length;
            return '<div class="diary-food-picker-bulk-bar" role="status">' +
                '<span>Выбрано: ' + count + '</span>' +
                '<button class="diary-food-picker-bulk-cancel" type="button" onclick="clearDiaryFoodPickerBulkSelection(event)">Отмена</button>' +
                '<button class="diary-food-picker-bulk-delete" type="button" onclick="deleteDiaryFoodPickerBulkSelection(event)">Удалить</button>' +
            '</div>';
        }

        function deleteDiaryFoodPickerBulkSelection(event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const type = diaryFoodPickerState.bulkSelectMode;
            const ids = getDiaryFoodPickerBulkSelectedIds(type);
            if (!type || !ids.length) return clearDiaryFoodPickerBulkSelection(event);
            if (!confirm('Удалить выбранное: ' + ids.length + '?')) return;
            const idSet = new Set(ids.map(String));
            if (type === 'quick') {
                saveQuickEntries(loadQuickEntries().filter(item => !idSet.has(String(item.id || ''))));
            } else if (type === 'products') {
                saveManualProducts(loadManualProducts().filter(product => !idSet.has(String(product.id || ''))));
                const favs = getDiaryMealFavorites('product').filter(id => !idSet.has(String(id)));
                localStorage.setItem(getDiaryMealFavoriteKey('product'), JSON.stringify(favs));
                ids.forEach(id => {
                    delete diaryFoodPickerState.selectedProducts?.[id];
                    delete diaryFoodPickerState.productDraftGrams?.[id];
                });
            } else if (type === 'recipes') {
                saveManualRecipes(loadManualRecipes().filter(recipe => !idSet.has(String(recipe.id || ''))));
                const favs = getDiaryMealFavorites('recipe').filter(id => !idSet.has(String(id)));
                localStorage.setItem(getDiaryMealFavoriteKey('recipe'), JSON.stringify(favs));
            }
            resetDiaryFoodPickerBulkSelection();
            diaryFoodPickerState.notice = ids.length === 1 ? 'Удалено.' : 'Удалено: ' + ids.length + '.';
            renderDiaryFoodPickerScreen();
        }
        ...
        function openDiaryFoodPickerQuickEntry(event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            resetDiaryFoodPickerBulkSelection();
            diaryFoodPickerState.quickCreateMode = true;
            diaryFoodPickerState.quickEditId = null;
            diaryFoodPickerState.quickCreateDraft = { name: '', grams: '', kcal: '', protein: '', fat: '', carbs: '' };
            diaryFoodPickerState.quickCreateError = '';
            diaryFoodPickerState.notice = '';
            renderDiaryFoodPickerScreen();
            setTimeout(() => document.getElementById('diary-food-picker-quick-name')?.focus?.(), 40);
        }

        function openDiaryFoodPickerQuickEdit(entryId, event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const entry = loadQuickEntries().find(item => String(item.id) === String(entryId));
            if (!entry) return showToast('Блюдо не найдено');
            diaryFoodPickerState.quickCreateMode = true;
            diaryFoodPickerState.quickEditId = String(entry.id);
            diaryFoodPickerState.quickCreateDraft = {
                name: entry.name || '',
                grams: entry.grams ? String(formatDiaryFoodPickerMacroValue(entry.grams)) : '',
                kcal: String(formatDiaryFoodPickerMacroValue(entry.kcal)),
                protein: String(formatDiaryFoodPickerMacroValue(entry.protein)),
                fat: String(formatDiaryFoodPickerMacroValue(entry.fat)),
                carbs: String(formatDiaryFoodPickerMacroValue(entry.carbs))
            };
            diaryFoodPickerState.quickCreateError = '';
            diaryFoodPickerState.notice = '';
            renderDiaryFoodPickerScreen();
        ...
        function renderDiaryFoodPickerQuickEntry() {
            if (diaryFoodPickerState.quickCreateMode) return renderDiaryFoodPickerQuickCreateForm();
            const entries = loadQuickEntries()
                .filter(entry => entry && entry.id && entry.name)
                .sort((a, b) => new Date(b.createdAt || b.created_at || 0) - new Date(a.createdAt || a.created_at || 0));
            const addButton = '<button class="diary-food-picker-create-product diary-food-picker-quick-add" type="button" onclick="openDiaryFoodPickerQuickEntry(event)">Добавить блюдо</button>';
            const bulkBar = renderDiaryFoodPickerBulkBar('quick');
            if (!entries.length) {
                return '<section class="diary-food-picker-quick-section">' + addButton +
                    bulkBar +
                    '<div class="diary-food-picker-empty diary-food-picker-quick-empty"><h3>Быстрых блюд пока нет</h3><p>Добавь блюдо, и оно появится здесь с датой создания.</p></div>' +
                '</section>';
            }
            return '<section class="diary-food-picker-quick-section">' + addButton +
                bulkBar +
                '<div class="diary-food-picker-quick-list">' + entries.map(renderDiaryFoodPickerQuickRow).join('') + '</div>' +
            '</section>';
        }

        function formatDiaryFoodPickerQuickDate(value) {
            try {
        ...
        function formatDiaryFoodPickerQuickLine(entry) {
            const grams = Number(entry?.grams) || 0;
            const kcal = Math.round(Number(entry?.kcal) || 0);
            const protein = formatDiaryFoodPickerMacroValue(Number(entry?.protein) || 0);
            const fat = formatDiaryFoodPickerMacroValue(Number(entry?.fat) || 0);
            const carbs = formatDiaryFoodPickerMacroValue(Number(entry?.carbs) || 0);
            return (grams > 0 ? Math.round(grams) + ' г • ' : '') + kcal + ' ккал • Б ' + protein + ' • Ж ' + fat + ' • У ' + carbs;
        }

        function renderDiaryFoodPickerQuickRow(entry) {
            const idArg = escapeAttr(JSON.stringify(String(entry.id || '')));
            const selected = isDiaryFoodPickerBulkSelected('quick', entry.id);
            const bulkMode = isDiaryFoodPickerBulkMode('quick');
            return '<article class="diary-food-picker-product-row diary-food-picker-quick-row is-bulk-selectable' + (bulkMode ? ' is-bulk-mode' : '') + (selected ? ' is-bulk-selected' : '') + '"' +
                ' onpointerdown="startDiaryFoodPickerBulkPress(\'quick\', ' + idArg + ', event)" onpointerup="cancelDiaryFoodPickerBulkPress()" onpointercancel="cancelDiaryFoodPickerBulkPress()" onpointerleave="cancelDiaryFoodPickerBulkPress()">' +
                '<div class="diary-food-picker-product-top">' +
                    '<div class="diary-food-picker-product-line">' +
                        '<div class="diary-food-picker-product-copy">' +
                            '<button class="diary-food-picker-product-name-btn" type="button" onclick="handleDiaryFoodPickerQuickNameClick(' + idArg + ', event)">' + escapeHTML(entry.name || 'Быстрое блюдо') + '</button>' +
                            '<small><span class="diary-food-picker-product-grams">' + escapeHTML(formatDiaryFoodPickerQuickDate(entry.createdAt || entry.created_at)) + '</span><span class="diary-food-picker-product-macros">' + escapeHTML(formatDiaryFoodPickerQuickLine(entry)) + '</span></small>' +
                        '</div>' +
                    '</div>' +
                    '<span aria-hidden="true"></span>' +
                    '<button class="diary-food-picker-product-check' + (bulkMode ? ' diary-food-picker-bulk-check' : '') + '" type="button" aria-label="' + (bulkMode ? (selected ? 'Снять выделение' : 'Выделить блюдо') : 'Добавить блюдо') + '" onclick="handleDiaryFoodPickerQuickActionClick(' + idArg + ', event)">' + (bulkMode ? (selected ? '✓' : '') : '+') + '</button>' +
                '</div>' +
            '</article>';
        }

        function handleDiaryFoodPickerQuickNameClick(entryId, event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            if (consumeDiaryFoodPickerBulkPress()) return;
            if (isDiaryFoodPickerBulkMode('quick')) return toggleDiaryFoodPickerBulkSelection('quick', entryId, event);
            openDiaryFoodPickerQuickEdit(entryId, event);
        }

        function handleDiaryFoodPickerQuickActionClick(entryId, event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            if (consumeDiaryFoodPickerBulkPress()) return;
            if (isDiaryFoodPickerBulkMode('quick')) return toggleDiaryFoodPickerBulkSelection('quick', entryId, event);
            addDiaryFoodPickerQuickEntry(entryId, event);
        }

        function renderDiaryFoodPickerQuickCreateForm() {
            const draft = diaryFoodPickerState.quickCreateDraft || {};
            const isEdit = Boolean(diaryFoodPickerState.quickEditId);
            const error = diaryFoodPickerState.quickCreateError
                ? '<div class="diary-food-picker-form-error" role="alert">' + escapeHTML(diaryFoodPickerState.quickCreateError) + '</div>'
                : '';
            return '<section class="diary-food-picker-product-form">' +
                '<div class="diary-food-picker-form-head"><h3>' + (isEdit ? 'Изменить блюдо' : 'Добавить блюдо') + '</h3><p>Укажи вес и КБЖУ для быстрого добавления в дневник.</p></div>' +
                error +
                '<label class="diary-food-picker-field diary-food-picker-field-wide"><span>Название блюда</span><input id="diary-food-picker-quick-name" type="text" value="' + escapeAttr(draft.name || '') + '" placeholder="Например, обед в кафе" autocomplete="off" oninput="updateDiaryFoodPickerQuickDraft(\'name\', this.value)"></label>' +
                '<div class="diary-food-picker-form-grid">' +
                    '<label class="diary-food-picker-field"><span>Вес, г</span><input type="number" inputmode="decimal" min="1" step="1" value="' + escapeAttr(draft.grams || '') + '" placeholder="100" oninput="updateDiaryFoodPickerQuickDraft(\'grams\', this.value)"></label>' +
                    '<label class="diary-food-picker-field"><span>Калории</span><input type="number" inputmode="decimal" min="0" step="0.1" value="' + escapeAttr(draft.kcal || '') + '" placeholder="0" oninput="updateDiaryFoodPickerQuickDraft(\'kcal\', this.value)"></label>' +
                    '<label class="diary-food-picker-field"><span>Белки</span><input type="number" inputmode="decimal" min="0" step="0.1" value="' + escapeAttr(draft.protein || '') + '" placeholder="0" oninput="updateDiaryFoodPickerQuickDraft(\'protein\', this.value)"></label>' +
                    '<label class="diary-food-picker-field"><span>Жиры</span><input type="number" inputmode="decimal" min="0" step="0.1" value="' + escapeAttr(draft.fat || '') + '" placeholder="0" oninput="updateDiaryFoodPickerQuickDraft(\'fat\', this.value)"></label>' +
                    '<label class="diary-food-picker-field"><span>Углеводы</span><input type="number" inputmode="decimal" min="0" step="0.1" value="' + escapeAttr(draft.carbs || '') + '" placeholder="0" oninput="updateDiaryFoodPickerQuickDraft(\'carbs\', this.value)"></label>' +
                '</div>' +
                '<div class="diary-food-picker-form-summary" id="diary-food-picker-quick-create-summary">' + escapeHTML(formatDiaryFoodPickerQuickDraftSummary(draft)) + '</div>' +
                '<div class="diary-food-picker-form-actions">' +
                    '<button class="diary-food-picker-form-cancel" type="button" onclick="cancelDiaryFoodPickerQuickCreate(event)">Отмена</button>' +
                    '<button class="diary-food-picker-form-save" type="button" onclick="saveDiaryFoodPickerQuickCreate(event)">' + (isEdit ? 'Сохранить изменения' : 'Сохранить') + '</button>' +
                '</div>' +
            '</section>';
        }
        ...
        function renderDiaryFoodPickerMyRecipes() {
            ...
            const bulkBar = renderDiaryFoodPickerBulkBar('recipes');
            if (!recipes.length) {
                return '<div class="diary-food-picker-empty"><h3>Пока нет собственных рецептов</h3><p>Создай рецепт, и он появится здесь для быстрого добавления.</p>' + createButton + bulkBar + '</div>';
            }
            return '<section class="diary-meal-recipe-section"><div class="diary-meal-section-title">Собственные рецепты</div>' + createButton + bulkBar + '<div class="recipes-results-grid recipe-grid diary-meal-catalog-grid">' + renderMyRecipeGrid(recipes, {
                onClick: 'quickAddMyRecipeToDiary',
                onEdit: 'openMyRecipeEditFromFoodPicker',
                bulkType: 'recipes',
                bulkMode: isDiaryFoodPickerBulkMode('recipes'),
                bulkSelectedIds: getDiaryFoodPickerBulkSelectedIds('recipes')
            }) + '</div></section>';
        }
        ...
        function calculateDiaryFoodPickerPortion(product, grams = 100) {
            if (grams === '' || grams === null || grams === undefined) {
                return { grams: 0, kcal: 0, protein: 0, fat: 0, carbs: 0 };
            }
            const safeGrams = Number(String(grams).replace(',', '.'));
            ...
            return {
                grams: safeGrams,
                kcal: (Number(product?.kcal) || 0) * ratio,
                protein: (Number(product?.protein) || 0) * ratio,
                fat: (Number(product?.fat) || 0) * ratio,
                carbs: (Number(product?.carbs) || 0) * ratio
            };
        }

        function getDiaryFoodPickerSelectedCount() {
            return Object.keys(diaryFoodPickerState.selectedProducts || {}).length;
        }

        function getDiaryFoodPickerSelectedProducts() {
            const selected = diaryFoodPickerState.selectedProducts || {};
            return Object.keys(selected)
                .map(key => {
                    const product = getDiaryFoodPickerProducts().find(item => String(item.id) === key) || selected[key]?.product;
                    if (!product) return null;
                    const rawGrams = selected[key]?.grams;
                    const grams = Number.isFinite(Number(String(rawGrams).replace(',', '.'))) && Number(String(rawGrams).replace(',', '.')) > 0
                        ? Number(String(rawGrams).replace(',', '.'))
                        : 100;
                    return { key, product, grams };
                })
                .filter(Boolean);
        }
        ...
        function formatDiaryFoodPickerPortionLine(portion) {
            return Math.round(Number(portion?.kcal) || 0) + ' ккал • Б ' +
                formatDiaryFoodPickerMacroValue(portion?.protein) + ' • Ж ' +
                formatDiaryFoodPickerMacroValue(portion?.fat) + ' • У ' +
                formatDiaryFoodPickerMacroValue(portion?.carbs);
        }
        ...
        function renderDiaryFoodPickerProductRow(product) {
            ...
            const selected = Boolean(diaryFoodPickerState.selectedProducts?.[productKey]);
            const bulkMode = isDiaryFoodPickerBulkMode('products');
            const bulkSelectable = product.source === 'manual';
            const bulkSelected = isDiaryFoodPickerBulkSelected('products', productKey);
            const expanded = selected && String(diaryFoodPickerState.expandedProductId || '') === productKey;
            const rawGrams = selected
                ? diaryFoodPickerState.selectedProducts?.[productKey]?.grams
                : 100;
            const grams = rawGrams === '' ? '' : Math.max(1, Number(String(rawGrams ?? 100).replace(',', '.')) || 100);
            const portion = calculateDiaryFoodPickerPortion(product, grams);
            const gramsLabel = grams === '' ? '0 г' : formatDiaryFoodPickerMacroValue(portion.grams) + ' г';
            return '<article class="diary-food-picker-product-row' + (selected ? ' is-selected' : '') + (expanded ? ' is-active' : '') + (bulkSelectable ? ' is-bulk-selectable' : '') + (bulkMode ? ' is-bulk-mode' : '') + (bulkSelected ? ' is-bulk-selected' : '') + '" data-food-picker-product-id="' + escapeAttr(productKey) + '"' +
                (bulkSelectable ? ' onpointerdown="startDiaryFoodPickerBulkPress(\'products\', ' + idArg + ', event)" onpointerup="cancelDiaryFoodPickerBulkPress()" onpointercancel="cancelDiaryFoodPickerBulkPress()" onpointerleave="cancelDiaryFoodPickerBulkPress()"' : '') + '>' +
                '<div class="diary-food-picker-product-top">' +
                    '<div class="diary-food-picker-product-line" role="button" tabindex="0" onclick="handleDiaryFoodPickerProductLineClick(' + idArg + ', event)" onkeydown="handleDiaryFoodPickerProductLineKeydown(' + idArg + ', event)">' +
                        '<span class="diary-food-picker-product-copy">' +
                            '<button class="diary-food-picker-product-name-btn" type="button" onclick="handleDiaryFoodPickerProductNameClick(' + idArg + ', event)">' + escapeHTML(product.name) + '</button>' +
                            '<small><span class="diary-food-picker-product-grams">' + escapeHTML(gramsLabel) + '</span><span class="diary-food-picker-product-macros">' + escapeHTML(formatDiaryFoodPickerPortionLine(portion)) + '</span></small>' +
                        '</span>' +
                    '</div>' +
                    '<button class="diary-food-picker-product-fav' + (favorite ? ' active' : '') + '" type="button" aria-label="' + (favorite ? 'Убрать из избранного' : 'Добавить в избранное') + '" aria-pressed="' + (favorite ? 'true' : 'false') + '" onclick="handleDiaryFoodPickerProductFavoriteClick(' + idArg + ', event)">' + (favorite ? '♥' : '♡') + '</button>' +
                    '<button class="diary-food-picker-product-check' + (bulkMode ? ' diary-food-picker-bulk-check' : '') + '" type="button" aria-label="' + (bulkMode ? (bulkSelected ? 'Снять выделение' : 'Выделить продукт') : (selected ? 'Снять выбор продукта' : 'Выбрать продукт')) + '" onclick="handleDiaryFoodPickerProductCheckClick(' + idArg + ', event)">' + (bulkMode ? (bulkSelected ? '✓' : '') : (selected ? '−' : '+')) + '</button>' +
                '</div>' +
                (expanded ? '<div class="diary-food-picker-portion-card">' +
                    '<div class="diary-food-picker-grams-stepper">' +
                        '<button class="diary-food-picker-grams-minus" type="button" aria-label="Уменьшить вес" onclick="adjustDiaryFoodPickerProductGrams(' + idArg + ', -10)">−</button>' +
                        '<label><input type="number" inputmode="decimal" min="1" step="1" value="' + escapeAttr(grams === '' ? '' : String(Math.round(grams))) + '" oninput="setDiaryFoodPickerProductGrams(' + idArg + ', this.value)"><span>г</span></label>' +
                        '<button class="diary-food-picker-grams-plus" type="button" aria-label="Увеличить вес" onclick="adjustDiaryFoodPickerProductGrams(' + idArg + ', 10)">+</button>' +
                    '</div>' +
                '</div>' : '') +
            '</article>';
        }

        function handleDiaryFoodPickerProductBulkIntent(productId, event) {
            if (consumeDiaryFoodPickerBulkPress()) return true;
            if (!isDiaryFoodPickerBulkMode('products')) return false;
            toggleDiaryFoodPickerBulkSelection('products', productId, event);
            return true;
        }

        function handleDiaryFoodPickerProductNameClick(productId, event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            if (handleDiaryFoodPickerProductBulkIntent(productId, event)) return;
            openDiaryFoodPickerEditProduct(productId, event);
        }

        function handleDiaryFoodPickerProductLineClick(productId, event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            if (handleDiaryFoodPickerProductBulkIntent(productId, event)) return;
            expandDiaryFoodPickerProduct(productId, event);
        }

        function handleDiaryFoodPickerProductFavoriteClick(productId, event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            if (handleDiaryFoodPickerProductBulkIntent(productId, event)) return;
            toggleDiaryFoodPickerProductFavorite(productId, event);
        }

        function handleDiaryFoodPickerProductCheckClick(productId, event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            if (handleDiaryFoodPickerProductBulkIntent(productId, event)) return;
            toggleDiaryFoodPickerProduct(productId, event);
        }

        function renderDiaryFoodPickerProducts() {
            if (diaryFoodPickerState.productCreateMode) return renderDiaryFoodPickerProductCreateForm();
            ...
            const clearButton = getDiaryFoodPickerSelectedCount() > 0
                ? '<button class="diary-food-picker-clear-selection" type="button" onclick="clearDiaryFoodPickerProductSelection(event)">Снять выбор</button>'
                : '';
            const bulkBar = renderDiaryFoodPickerBulkBar('products');
            const notice = diaryFoodPickerState.notice
                ? '<div class="diary-food-picker-notice" role="status">' + escapeHTML(diaryFoodPickerState.notice) + '</div>'
                : '';
            ...
            return '<div class="diary-food-picker-search-row"><label class="diary-food-picker-search"><input id="diary-food-picker-product-search" type="search" value="' + escapeAttr(query) + '" placeholder="Найти продукт" autocomplete="off" oninput="handleDiaryFoodPickerProductSearch(this.value)"></label>' +
                ...
                notice + bulkBar + clearButton + body;
        }

        function renderDiaryFoodPickerProductCreateForm() {
            const draft = diaryFoodPickerState.productCreateDraft || {};
            const isEdit = Boolean(diaryFoodPickerState.productEditMode);
            const error = diaryFoodPickerState.productCreateError
                ? '<div class="diary-food-picker-form-error" role="alert">' + escapeHTML(diaryFoodPickerState.productCreateError) + '</div>'
                : '';
            return '<section class="diary-food-picker-product-form">' +
                '<div class="diary-food-picker-form-head"><h3>' + (isEdit ? 'Изменить продукт' : 'Добавить новый продукт') + '</h3><p>Укажи КБЖУ на 100 г. После сохранения продукт появится в списке.</p></div>' +
                error +
                '<label class="diary-food-picker-field diary-food-picker-field-wide"><span>Название продукта</span><input id="diary-food-picker-product-name" type="text" value="' + escapeAttr(draft.name || '') + '" placeholder="Например, куриная грудка" autocomplete="off" oninput="updateDiaryFoodPickerProductDraft(\'name\', this.value)"></label>' +
                '<div class="diary-food-picker-form-grid">' +
                    '<label class="diary-food-picker-field"><span>Калории / 100 г</span><input type="number" inputmode="decimal" min="0" step="0.1" value="' + escapeAttr(draft.kcal || '') + '" placeholder="0" oninput="updateDiaryFoodPickerProductDraft(\'kcal\', this.value)"></label>' +
                    '<label class="diary-food-picker-field"><span>Белки / 100 г</span><input type="number" inputmode="decimal" min="0" step="0.1" value="' + escapeAttr(draft.protein || '') + '" placeholder="0" oninput="updateDiaryFoodPickerProductDraft(\'protein\', this.value)"></label>' +
                    '<label class="diary-food-picker-field"><span>Жиры / 100 г</span><input type="number" inputmode="decimal" min="0" step="0.1" value="' + escapeAttr(draft.fat || '') + '" placeholder="0" oninput="updateDiaryFoodPickerProductDraft(\'fat\', this.value)"></label>' +
                    '<label class="diary-food-picker-field"><span>Углеводы / 100 г</span><input type="number" inputmode="decimal" min="0" step="0.1" value="' + escapeAttr(draft.carbs || '') + '" placeholder="0" oninput="updateDiaryFoodPickerProductDraft(\'carbs\', this.value)"></label>' +
                '</div>' +
                '<div class="diary-food-picker-form-summary" id="diary-food-picker-product-create-summary">На 100 г: ' + escapeHTML(formatDiaryFoodPickerProductCreateSummary(draft)) + '</div>' +
                '<div class="diary-food-picker-form-actions">' +
                    '<button class="diary-food-picker-form-cancel" type="button" onclick="cancelDiaryFoodPickerCreateProduct(event)">Отмена</button>' +
                    '<button class="diary-food-picker-form-save" type="button" onclick="saveDiaryFoodPickerCreateProduct(event)">' + (isEdit ? 'Сохранить изменения' : 'Сохранить') + '</button>' +
                '</div>' +
            '</section>';
        }
        ...
        function toggleDiaryFoodPickerProduct(productId, event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const key = String(productId || '');
            const product = getDiaryFoodPickerProducts().find(item => String(item.id) === key);
            if (!product) return showToast('Продукт не найден');
            diaryFoodPickerState.notice = '';
            if (!diaryFoodPickerState.selectedProducts) diaryFoodPickerState.selectedProducts = {};
            if (diaryFoodPickerState.selectedProducts[key]) {
                delete diaryFoodPickerState.selectedProducts[key];
                if (diaryFoodPickerState.productDraftGrams) delete diaryFoodPickerState.productDraftGrams[key];
                if (String(diaryFoodPickerState.expandedProductId || '') === key) diaryFoodPickerState.expandedProductId = null;
            } else {
                if (!diaryFoodPickerState.productDraftGrams) diaryFoodPickerState.productDraftGrams = {};
                const grams = diaryFoodPickerState.productDraftGrams[key] || 100;
                diaryFoodPickerState.selectedProducts[key] = { product, grams };
                diaryFoodPickerState.productDraftGrams[key] = grams;
                diaryFoodPickerState.expandedProductId = key;
            }
            renderDiaryFoodPickerScreen();
        }

        function expandDiaryFoodPickerProduct(productId, event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const key = String(productId || '');
            if (!diaryFoodPickerState.selectedProducts?.[key]) return;
            diaryFoodPickerState.expandedProductId = key;
            renderDiaryFoodPickerScreen();
        }
        ...
        function handleDiaryFoodPickerProductLineKeydown(productId, event) {
            ...
            handleDiaryFoodPickerProductLineClick(productId, event);
        }

        function clearDiaryFoodPickerProductSelection(event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            diaryFoodPickerState.selectedProducts = {};
            diaryFoodPickerState.expandedProductId = null;
            diaryFoodPickerState.productDraftGrams = {};
            diaryFoodPickerState.notice = '';
            renderDiaryFoodPickerScreen();
        }

        function setDiaryFoodPickerProductGrams(productId, value) {
            const key = String(productId || '');
            if (!key) return;
            const grams = String(value || '') === '' ? '' : Math.max(1, Number(String(value).replace(',', '.')) || 1);
            if (!diaryFoodPickerState.productDraftGrams) diaryFoodPickerState.productDraftGrams = {};
            diaryFoodPickerState.productDraftGrams[key] = grams;
            if (diaryFoodPickerState.selectedProducts?.[key]) diaryFoodPickerState.selectedProducts[key].grams = grams;
            const product = diaryFoodPickerState.selectedProducts?.[key]?.product || getDiaryFoodPickerProducts().find(item => String(item.id) === key);
            if (!product) return;
            const portion = calculateDiaryFoodPickerPortion(product, grams);
            const row = Array.from(document.querySelectorAll('.diary-food-picker-product-row')).find(item => String(item.dataset.foodPickerProductId || '') === key);
            const gramsEl = row?.querySelector('.diary-food-picker-product-grams');
            if (gramsEl) gramsEl.textContent = (grams === '' ? '0' : formatDiaryFoodPickerMacroValue(portion.grams)) + ' г';
            const macrosEl = row?.querySelector('.diary-food-picker-product-macros');
            if (macrosEl) macrosEl.textContent = formatDiaryFoodPickerPortionLine(portion);
        }

        function adjustDiaryFoodPickerProductGrams(productId, delta) {
            const key = String(productId || '');
            const current = Number(String(diaryFoodPickerState.selectedProducts?.[key]?.grams ?? diaryFoodPickerState.productDraftGrams?.[key] ?? 100).replace(',', '.')) || 0;
            const next = Math.max(1, current + Number(delta || 0));
            const row = Array.from(document.querySelectorAll('.diary-food-picker-product-row')).find(item => String(item.dataset.foodPickerProductId || '') === key);
            const input = row?.querySelector('.diary-food-picker-grams-stepper input');
            if (input) input.value = String(Math.round(next));
            setDiaryFoodPickerProductGrams(key, next);
        }

        async function saveDiaryFoodPickerSelectedProducts(event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            if (isAddingMeal) return;
            const selectedItems = getDiaryFoodPickerSelectedProducts();
            if (!selectedItems.length) return closeDiaryFoodPickerScreen();
            const mealType = getValidMealType(diaryFoodPickerState.mealType, currentDiaryMealType || 'Завтрак');

            isAddingMeal = true;
            diaryFoodPickerState.notice = 'Добавляю выбранное в ' + mealType + '...';
            renderDiaryFoodPickerScreen();
            const optimisticIds = [];
            try {
                for (const item of selectedItems) {
                    const product = item.product;
                    const portion = calculateDiaryFoodPickerPortion(product, item.grams);
                    const payload = buildDiaryManualProductPayload({
                        id: product.id || null,
                        name: product.name || 'Продукт',
                        grams: portion.grams || 100,
                        caloriesPer100: product.kcal,
                        proteinPer100: product.protein,
                        fatPer100: product.fat,
                        carbsPer100: product.carbs
                    }, mealType);
                    payload.productId = product.id || payload.productId || null;
                    optimisticIds.push(addOptimisticDiaryRow({
                        name: product.name,
                        k: payload.kcal,
                        p: payload.protein,
                        f: payload.fat,
                    ...
                    upsertManualProduct(product.name, {
                        kcal100: product.kcal,
                        protein100: product.protein,
                        fat100: product.fat,
                        carbs100: product.carbs
                    });
                    recordProductSmartAction(product.id || product.name, mealType);
                }
                diaryFoodPickerState.selectedProducts = {};
                diaryFoodPickerState.expandedProductId = null;
                diaryFoodPickerState.notice = '';
                ...
                document.getElementById('history-list')?.classList.add('success-flash');
                setTimeout(() => document.getElementById('history-list')?.classList.remove('success-flash'), 700);
                showToast('Добавлено продуктов: ' + selectedItems.length);
            } catch (error) {
                optimisticIds.forEach(removeOptimisticDiaryRow);
                console.error('Ошибка добавления продукта из Food Picker:', error);
                diaryFoodPickerState.notice = 'Не удалось добавить продукт: ' + (error.message || error);
                renderDiaryFoodPickerScreen();
            } finally {
                isAddingMeal = false;
            }
        ...
        function openDiaryFoodPickerCreateProduct() {
            resetDiaryFoodPickerBulkSelection();
            diaryFoodPickerState.productCreateMode = true;
            diaryFoodPickerState.productEditMode = false;
            diaryFoodPickerState.productEditId = null;
            diaryFoodPickerState.productCreateDraft = { name: '', kcal: '', protein: '', fat: '', carbs: '' };
            diaryFoodPickerState.productCreateError = '';
            diaryFoodPickerState.notice = '';
            renderDiaryFoodPickerScreen();
            setTimeout(() => document.getElementById('diary-food-picker-product-name')?.focus?.(), 40);
        }

        function openDiaryFoodPickerEditProduct(productId, event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const key = String(productId || '');
            const product = getDiaryFoodPickerProducts().find(item => String(item.id) === key);
            if (!product) return showToast('Продукт не найден');
            diaryFoodPickerState.productCreateMode = true;
            diaryFoodPickerState.productEditMode = true;
            diaryFoodPickerState.productEditId = product.source === 'manual' ? key : null;
            diaryFoodPickerState.productCreateDraft = {
                name: product.name || '',
                kcal: String(formatDiaryFoodPickerMacroValue(product.kcal)),
                protein: String(formatDiaryFoodPickerMacroValue(product.protein)),
                fat: String(formatDiaryFoodPickerMacroValue(product.fat)),
                carbs: String(formatDiaryFoodPickerMacroValue(product.carbs))
            };
            diaryFoodPickerState.productCreateError = '';
            diaryFoodPickerState.notice = product.source === 'manual'
                ? ''
                : 'Это базовый продукт. После сохранения он появится в твоих продуктах.';
            renderDiaryFoodPickerScreen();
            setTimeout(() => document.getElementById('diary-food-picker-product-name')?.focus?.(), 40);
        }

        function cancelDiaryFoodPickerCreateProduct(event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
        ...
        function updateDiaryFoodPickerProductDraft(field, value) {
            if (!diaryFoodPickerState.productCreateDraft) diaryFoodPickerState.productCreateDraft = {};
            diaryFoodPickerState.productCreateDraft[field] = String(value || '');
            diaryFoodPickerState.productCreateError = '';
            const summary = document.getElementById('diary-food-picker-product-create-summary');
            if (summary) summary.textContent = 'На 100 г: ' + formatDiaryFoodPickerProductCreateSummary(diaryFoodPickerState.productCreateDraft);
            const error = document.querySelector('.diary-food-picker-form-error');
            if (error) error.remove();
        }

        function parseDiaryFoodPickerProductNumber(value) {
            if (value === '' || value === null || value === undefined) return 0;
            const number = Number(String(value).replace(',', '.'));
            return Number.isFinite(number) ? number : 0;
        }

        function hasNegativeDiaryFoodPickerProductNumber(value) {
            if (value === '' || value === null || value === undefined) return false;
            const number = Number(String(value).replace(',', '.'));
            return Number.isFinite(number) && number < 0;
        }

        function formatDiaryFoodPickerProductCreateSummary(draft = {}) {
            const kcal = parseDiaryFoodPickerProductNumber(draft.kcal);
            const protein = parseDiaryFoodPickerProductNumber(draft.protein);
            const fat = parseDiaryFoodPickerProductNumber(draft.fat);
            const carbs = parseDiaryFoodPickerProductNumber(draft.carbs);
            return Math.round(kcal) + ' ккал • Б ' + formatDiaryFoodPickerMacroValue(protein) +
                ' • Ж ' + formatDiaryFoodPickerMacroValue(fat) +
                ' • У ' + formatDiaryFoodPickerMacroValue(carbs);
        }

        function formatDiaryFoodPickerQuickDraftSummary(draft = {}) {
            const grams = parseDiaryFoodPickerProductNumber(draft.grams);
            const kcal = parseDiaryFoodPickerProductNumber(draft.kcal);
            const protein = parseDiaryFoodPickerProductNumber(draft.protein);
            const fat = parseDiaryFoodPickerProductNumber(draft.fat);
            const carbs = parseDiaryFoodPickerProductNumber(draft.carbs);
            return (grams > 0 ? Math.round(grams) + ' г • ' : '') + Math.round(kcal) + ' ккал • Б ' + formatDiaryFoodPickerMacroValue(protein) +
                ' • Ж ' + formatDiaryFoodPickerMacroValue(fat) +
                ' • У ' + formatDiaryFoodPickerMacroValue(carbs);
        }

        function updateDiaryFoodPickerQuickDraft(field, value) {
            if (!diaryFoodPickerState.quickCreateDraft) diaryFoodPickerState.quickCreateDraft = {};
            diaryFoodPickerState.quickCreateDraft[field] = String(value || '');
            diaryFoodPickerState.quickCreateError = '';
            const summary = document.getElementById('diary-food-picker-quick-create-summary');
            if (summary) summary.textContent = formatDiaryFoodPickerQuickDraftSummary(diaryFoodPickerState.quickCreateDraft);
            const error = document.querySelector('.diary-food-picker-form-error');
            if (error) error.remove();
        }

        function cancelDiaryFoodPickerQuickCreate(event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            diaryFoodPickerState.quickCreateMode = false;
            diaryFoodPickerState.quickEditId = null;
            diaryFoodPickerState.quickCreateError = '';
            renderDiaryFoodPickerScreen();
        }

        function saveDiaryFoodPickerQuickCreate(event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const draft = diaryFoodPickerState.quickCreateDraft || {};
            const name = String(draft.name || '').trim().replace(/\s+/g, ' ');
            if (!name) {
                diaryFoodPickerState.quickCreateError = 'Введите название блюда.';
                renderDiaryFoodPickerScreen();
                setTimeout(() => document.getElementById('diary-food-picker-quick-name')?.focus?.(), 40);
                return;
            }
            const fields = ['grams', 'kcal', 'protein', 'fat', 'carbs'];
            if (fields.some(field => hasNegativeDiaryFoodPickerProductNumber(draft[field]))) {
                diaryFoodPickerState.quickCreateError = 'Вес и КБЖУ не должны быть отрицательными.';
                renderDiaryFoodPickerScreen();
                return;
            }
            const grams = parseDiaryFoodPickerProductNumber(draft.grams);
            const existingEntries = loadQuickEntries();
            const editId = String(diaryFoodPickerState.quickEditId || '');
            const entry = {
                id: editId || ('quick_entry_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8)),
                type: 'quick-entry-template',
                name,
                grams: grams > 0 ? grams : 0,
                kcal: parseDiaryFoodPickerProductNumber(draft.kcal),
                protein: parseDiaryFoodPickerProductNumber(draft.protein),
                fat: parseDiaryFoodPickerProductNumber(draft.fat),
                carbs: parseDiaryFoodPickerProductNumber(draft.carbs),
                createdAt: existingEntries.find(item => String(item.id) === editId)?.createdAt || new Date().toISOString(),
                updatedAt: editId ? new Date().toISOString() : undefined
            };
            const nextEntries = editId
                ? existingEntries.map(item => String(item.id) === editId ? { ...item, ...entry } : item)
                : [entry].concat(existingEntries).slice(0, 80);
            saveQuickEntries(nextEntries);
            diaryFoodPickerState.quickCreateMode = false;
            diaryFoodPickerState.quickEditId = null;
            diaryFoodPickerState.quickCreateDraft = { name: '', grams: '', kcal: '', protein: '', fat: '', carbs: '' };
            diaryFoodPickerState.quickCreateError = '';
            diaryFoodPickerState.notice = editId ? 'Блюдо обновлено.' : 'Блюдо сохранено в быстром вводе.';
            renderDiaryFoodPickerScreen();
        }

        async function addDiaryFoodPickerQuickEntry(entryId, event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            if (isAddingMeal) return;
            const entry = loadQuickEntries().find(item => String(item.id) === String(entryId));
            ...
            const createdAt = selectedDateTimeISO();
            const payload = {
                id: 'quick_entry_meal_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8),
                type: 'quick-entry',
                name: entry.name || 'Быстрое блюдо',
                recipe_title: entry.name || 'Быстрое блюдо',
                mealType,
                meal_type: mealType,
            ...
            isAddingMeal = true;
            const optimisticId = addOptimisticDiaryRow({ name: payload.name, k: payload.kcal, p: payload.protein, f: payload.fat, c: payload.carbs }, mealType);
            try {
                try {
                    await callServer('addMeal', payload);
                } catch (error) {
                ...
                removeOptimisticDiaryRow(optimisticId);
                console.error('Ошибка добавления быстрого блюда:', error);
                diaryFoodPickerState.notice = 'Не удалось добавить блюдо: ' + (error.message || error);
                renderDiaryFoodPickerScreen();
            } finally {
                isAddingMeal = false;
            }
        ...
        function saveDiaryFoodPickerCreateProduct(event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const draft = diaryFoodPickerState.productCreateDraft || {};
            const name = String(draft.name || '').trim().replace(/\s+/g, ' ');
            if (!name) {
                diaryFoodPickerState.productCreateError = 'Введите название продукта.';
                renderDiaryFoodPickerScreen();
                setTimeout(() => document.getElementById('diary-food-picker-product-name')?.focus?.(), 40);
                return;
            }
            ...
            if (fields.some(field => hasNegativeDiaryFoodPickerProductNumber(draft[field]))) {
                diaryFoodPickerState.productCreateError = 'КБЖУ не должны быть отрицательными.';
                renderDiaryFoodPickerScreen();
                return;
            }
            const totals = {
                kcal100: parseDiaryFoodPickerProductNumber(draft.kcal),
                protein100: parseDiaryFoodPickerProductNumber(draft.protein),
                fat100: parseDiaryFoodPickerProductNumber(draft.fat),
                carbs100: parseDiaryFoodPickerProductNumber(draft.carbs)
            };
            let saved = null;
            if (diaryFoodPickerState.productEditMode && diaryFoodPickerState.productEditId) {
                const now = new Date().toISOString();
                const editId = String(diaryFoodPickerState.productEditId);
                const products = loadManualProducts().filter(isValidManualProduct);
                const index = products.findIndex(product => String(product.id) === editId);
                if (index >= 0) {
                    products[index] = {
                        ...products[index],
                        name,
            ...
            if (!saved) saved = upsertManualProduct(name, totals);
            if (!saved) {
                diaryFoodPickerState.productCreateError = 'Не удалось сохранить продукт. Проверьте значения КБЖУ.';
                renderDiaryFoodPickerScreen();
                return;
            }
            diaryFoodPickerState.productCreateMode = false;
            ...
            diaryFoodPickerState.productCreateDraft = { name: '', kcal: '', protein: '', fat: '', carbs: '' };
            diaryFoodPickerState.productCreateError = '';
            diaryFoodPickerState.productQuery = '';
            diaryFoodPickerState.productsFavoritesOnly = false;
            if (!diaryFoodPickerState.selectedProducts) diaryFoodPickerState.selectedProducts = {};
            if (!diaryFoodPickerState.productDraftGrams) diaryFoodPickerState.productDraftGrams = {};
            if (saved.id) {
                const key = String(saved.id);
                const product = mapManualProductToDiaryFoodPickerProduct(saved);
                const grams = diaryFoodPickerState.selectedProducts[key]?.grams ?? diaryFoodPickerState.productDraftGrams[key] ?? 100;
                diaryFoodPickerState.productDraftGrams[key] = grams;
                diaryFoodPickerState.selectedProducts[key] = { product, grams };
                diaryFoodPickerState.expandedProductId = key;
            }
            diaryFoodPickerState.notice = 'Продукт сохранён. Можно указать граммы и добавить в дневник.';
            renderDiaryFoodPickerScreen();
        }
        ...
        function renderDiaryFoodPickerScreen() {
            ...
            const panels = DIARY_FOOD_PICKER_TABS.map(tabName => {
                const active = tabName === activeTab;
                const content = tabName === 'quick'
                    ? renderDiaryFoodPickerQuickEntry()
                    : tabName === 'products'
                        ? renderDiaryFoodPickerProducts()
                        : renderDiaryFoodPickerPlaceholder(tabName);
                return '<section class="diary-food-picker-panel' + (active ? ' active' : '') + '" role="tabpanel">' + content + '</section>';
            }).join('');
            const selectedCount = getDiaryFoodPickerSelectedCount();
            const actionButton = selectedCount > 0
                ? '<button class="diary-food-picker-cancel is-save" type="button" onclick="saveDiaryFoodPickerSelectedProducts(event)">Сохранить (' + selectedCount + ')</button>'
        ...
        function normalizeFoodSearchText(value) {
            return normalizeManualProductName(value).replace(/ё/g, 'е');
        }

        function decodeFoodSearchEntities(value) {
        ...
        function cleanFoodSearchDisplayText(value) {
            return decodeFoodSearchEntities(value)
                .replace(/<br\s*\/?>/gi, ' ')
                .replace(/<[^>]*>/g, ' ')
                .replace(/[\u0000-\u001F\u007F-\u009F\u200B-\u200D\uFEFF\uFFFC\uFFFD]/g, ' ')
                .replace(/\s+/g, ' ')
        ...
        function normalizeFoodSearchDisplayName(value) {
            return cleanFoodSearchDisplayText(value).replace(/^([^A-Za-zА-Яа-яЁё]*)([a-zа-яё])/, (_, prefix, letter) => prefix + letter.toUpperCase());
        }

        function isLikelyUkrainianFoodSearchResult(value) {
            const text = cleanFoodSearchDisplayText(value).toLowerCase().replace(/\s+/g, ' ');
        ...
        function hasValidSearchProductNutrition(product) {
            const kcal = Number(product?.caloriesPer100);
            const protein = Number(product?.proteinPer100);
            const fat = Number(product?.fatPer100);
            const carbs = Number(product?.carbsPer100);
            if (![kcal, protein, fat, carbs].every(Number.isFinite)) return false;
            if (kcal <= 0 || kcal > 950) return false;
            if ([protein, fat, carbs].some(value => value < 0 || value > 100)) return false;
            return protein + fat + carbs <= 120;
        }

        function hasFoodSearchQuery() {
            return normalizeFoodSearchText(document.getElementById('diary-meal-search-input')?.value || '').length >= FOOD_SEARCH_MIN_QUERY_LENGTH;
        }

        function mapSearchProduct(raw, source) {
            if (!raw) return null;
            const name = normalizeFoodSearchDisplayName(raw.name || '');
            if (!name) return null;
            const brand = cleanFoodSearchDisplayText(raw.brand || '');
            if (source === 'off' && isLikelyUkrainianFoodSearchResult(name + ' ' + brand)) return null;
            const product = {
                id: raw.id || (source + '_' + normalizeFoodSearchText(name)),
                name,
                barcode: normalizeBarcode(raw.barcode || ''),
                brand,
                caloriesPer100: normalizeManualProductNumber(raw.caloriesPer100 ?? raw.kcal100 ?? raw.kcal ?? raw.kcal_per_100),
                proteinPer100: normalizeManualProductNumber(raw.proteinPer100 ?? raw.protein100 ?? raw.protein ?? raw.protein_per_100),
                fatPer100: normalizeManualProductNumber(raw.fatPer100 ?? raw.fat100 ?? raw.fat ?? raw.fat_per_100),
                carbsPer100: normalizeManualProductNumber(raw.carbsPer100 ?? raw.carbs100 ?? raw.carbs ?? raw.carbs_per_100),
                imageUrl: safeImageUrl(raw.imageUrl || raw.image_url || raw.image || raw.image_front_small_url || raw.image_small_url || raw.image_front_url || ''),
                source,
                sourceLabel: source === 'manual' ? 'Мой продукт' : source === 'mypie' ? 'MyPie' : 'Open Food Facts'
            };
            return isValidManualProduct(product) && hasValidSearchProductNutrition(product) ? product : null;
        }

        function buildSearchProductKey(product) {
            return normalizeFoodSearchText(product?.name || '');
        }

        function mergeFoodSearchResults(groups) {
            ...
            return Array.from(merged.values()).slice(0, 18).map((product, index) => ({
                ...product,
                searchId: 'food_search_' + index + '_' + (product.source || 'item')
            }));
        }

        function searchUserProducts(query) {
            const normalized = normalizeFoodSearchText(query);
            return getSortedManualProducts()
                .filter(product => normalizeFoodSearchText(product.name).includes(normalized))
                .slice(0, 12)
                .map(product => mapSearchProduct(product, 'manual'))
                .filter(Boolean);
        }

        function searchLocalFoodProducts(query) {
            const normalized = normalizeFoodSearchText(query);
            if (!normalized) return [];
            return LOCAL_FOOD_SEARCH_PRODUCTS
                .filter(product => normalizeFoodSearchText(product.name).includes(normalized))
                .slice(0, 12)
                .map(product => mapSearchProduct(product, 'mypie'))
                .filter(Boolean);
        }

        function getMyPieProductsFromRecipes(query) {
            const normalized = normalizeFoodSearchText(query);
            const productsByName = new Map();
            recipesData.forEach(recipe => (recipe.recipe_ingredients || []).forEach(ingredient => {
                const product = ingredient.products || {};
                const mapped = mapSearchProduct({
                    id: product.id || ingredient.ingredient_id || product.name,
                    name: product.name || ingredient.name,
                    kcal: product.kcal ?? product.kcal_per_100 ?? ingredient.kcalPer100,
                    protein: product.protein ?? product.protein_per_100 ?? ingredient.proteinPer100,
                    fat: product.fat ?? product.fat_per_100 ?? ingredient.fatPer100,
                    carbs: product.carbs ?? product.carbs_per_100 ?? ingredient.carbsPer100
                }, 'mypie');
                const key = buildSearchProductKey(mapped);
                if (mapped && key.includes(normalized) && !productsByName.has(key)) productsByName.set(key, mapped);
            }));
            return Array.from(productsByName.values()).slice(0, 12);
        }

        async function searchMyPieProducts(query, signal) {
            const recipeProducts = getMyPieProductsFromRecipes(query);
            if (!supabaseClient) return recipeProducts;
            try {
                const pattern = '%' + String(query || '').trim().replace(/[%_]/g, '') + '%';
                let request = supabaseClient
                    .from('ingredients')
                    .select('id, name, category, kcal_per_100, protein_per_100, fat_per_100, carbs_per_100')
                    .ilike('name', pattern)
                ...
                return (data || [])
                    ...
                    .filter(Boolean)
                    .concat(recipeProducts);
            } catch (error) {
                if (signal?.aborted) return [];
                console.warn('[food search] MyPie search unavailable:', error);
                return recipeProducts;
            }
        }

        function mapOpenFoodFactsSearchProduct(row) {
            const nutriments = row?.nutriments || {};
            return mapSearchProduct({
                id: row?.code || row?._id || row?.id,
                barcode: row?.code || row?._id || '',
                name: row?.product_name_ru || row?.product_name || row?.generic_name_ru || row?.generic_name || '',
                brand: row?.brands || '',
                imageUrl: row?.image_front_small_url || row?.image_small_url || row?.image_front_url || row?.image_url || '',
                kcal100: getOpenFoodFactsKcal(row),
                protein100: nutriments.proteins_100g,
                fat100: nutriments.fat_100g,
                carbs100: nutriments.carbohydrates_100g
            }, 'off');
        }

        async function searchOpenFoodFactsProducts(query, signal) {
            try {
                const params = new URLSearchParams({
                    ...
                    json: '1',
                    page_size: '12',
                    fields: 'code,product_name,product_name_ru,generic_name,generic_name_ru,brands,image_front_small_url,image_small_url,image_front_url,image_url,nutriments'
                });
                ...
                const data = await response.json();
                return (data?.products || []).map(mapOpenFoodFactsSearchProduct).filter(Boolean);
            } catch (error) {
                if (signal?.aborted) return [];
                console.warn('[food search] Open Food Facts unavailable:', error);
                return [];
            }
        }

        function cancelDiaryFoodSearch() {
            if (diaryFoodSearchDebounceId) clearTimeout(diaryFoodSearchDebounceId);
            diaryFoodSearchDebounceId = 0;
            diaryFoodSearchController?.abort?.();
            diaryFoodSearchController = null;
        }

        function handleDiaryMealSearchInput() {
            const query = normalizeFoodSearchText(document.getElementById('diary-meal-search-input')?.value || '');
            if (diaryMealActiveTab !== 'products') {
                cancelDiaryFoodSearch();
                renderDiaryMealContent();
                return;
            }
            if (query.length < FOOD_SEARCH_MIN_QUERY_LENGTH) {
                cancelDiaryFoodSearch();
                diaryFoodSearchState = { ...diaryFoodSearchState, query, status: 'idle', results: [] };
                renderDiaryMealContent();
                return;
            }
            cancelDiaryFoodSearch();
            const instantResults = mergeFoodSearchResults([searchUserProducts(query), searchLocalFoodProducts(query), getMyPieProductsFromRecipes(query)]);
            diaryFoodSearchState = { ...diaryFoodSearchState, query, status: instantResults.length ? 'ready' : 'loading', results: instantResults, requestId: diaryFoodSearchState.requestId + 1 };
            const requestId = diaryFoodSearchState.requestId;
            renderDiaryMealContent();
            if (FOOD_SEARCH_DEBOUNCE_MS > 0) diaryFoodSearchDebounceId = setTimeout(() => performDiaryFoodSearch(query, requestId), FOOD_SEARCH_DEBOUNCE_MS);
            else performDiaryFoodSearch(query, requestId);
        }

        async function performDiaryFoodSearch(query, requestId) {
            diaryFoodSearchController = new AbortController();
            const signal = diaryFoodSearchController.signal;
            try {
                const userProducts = searchUserProducts(query);
                const localProducts = searchLocalFoodProducts(query);
                const [mypieProducts, offProducts] = await Promise.all([
                    searchMyPieProducts(query, signal),
                    searchOpenFoodFactsProducts(query, signal)
                ]);
                if (signal.aborted || requestId !== diaryFoodSearchState.requestId) return;
                diaryFoodSearchState = {
                    query,
                    status: 'ready',
                    results: mergeFoodSearchResults([userProducts, mypieProducts, localProducts, offProducts]),
                    requestId
                };
            } catch (error) {
                if (signal.aborted || requestId !== diaryFoodSearchState.requestId) return;
                console.warn('[food search] Search failed:', error);
                diaryFoodSearchState = { query, status: 'ready', results: mergeFoodSearchResults([searchUserProducts(query), searchLocalFoodProducts(query)]), requestId };
            } finally {
                if (requestId === diaryFoodSearchState.requestId) {
                    diaryFoodSearchController = null;
                    renderDiaryMealContent();
                }
            }
        ...
        function getDiaryFoodSearchResult(searchId) {
            return (diaryFoodSearchState.results || []).find(product => String(product.searchId) === String(searchId)) || null;
        }

        function openDiaryMealSearchResult(searchId) {
            const product = getDiaryFoodSearchResult(searchId);
            if (!product) return showToast('Продукт не найден');
            currentMealFilter = currentDiaryMealType;
            openProductSearchMealModal(product);
        }

        function clearQuickAddLongPress() {
            if (quickAddLongPressTimer) clearTimeout(quickAddLongPressTimer);
            quickAddLongPressTimer = 0;
        }

        function startDiaryMealProductAddPress(event, source, id) {
            event?.stopPropagation?.();
            clearQuickAddLongPress();
            quickAddLongPressHandled = false;
            quickAddLongPressTimer = setTimeout(() => {
                quickAddLongPressHandled = true;
                if (source === 'search') openDiaryMealSearchResult(id);
                else openDiaryMealProduct(id);
            }, 460);
        }

        function startRecipeQuickAddPress(event, recipeId) {
            event?.stopPropagation?.();
            clearQuickAddLongPress();
            quickAddLongPressHandled = false;
            quickAddLongPressTimer = setTimeout(() => {
                quickAddLongPressHandled = true;
                openRecipeQuickActions(recipeId, event);
            }, 460);
        }

        function startRecipeCardQuickActionPress(event, recipeId) {
            if (event?.target?.closest?.('[data-action], button, a, input, select, textarea')) return;
            clearQuickAddLongPress();
            quickAddLongPressHandled = false;
            quickAddLongPressTimer = setTimeout(() => {
                quickAddLongPressHandled = true;
        ...
        function finishQuickAddPress() {
            clearQuickAddLongPress();
        }

        function consumeQuickAddLongPress() {
            const handled = quickAddLongPressHandled;
            quickAddLongPressHandled = false;
            return handled;
        }

        function ensureRecipeQuickActionSheet() {
            let overlay = document.getElementById('recipe-quick-actions-overlay');
            if (overlay) return overlay;
            overlay = document.createElement('div');
            overlay.id = 'recipe-quick-actions-overlay';
            overlay.className = 'recipe-quick-actions-overlay';
            overlay.onclick = closeRecipeQuickActions;
            overlay.innerHTML = '<div class="recipe-quick-actions-sheet" onclick="event.stopPropagation()">' +
                '<div class="recipe-quick-actions-grabber" aria-hidden="true"></div>' +
                '<div class="recipe-quick-actions-title" id="recipe-quick-actions-title">Быстро добавить</div>' +
                '<div class="recipe-quick-actions-subtitle" id="recipe-quick-actions-subtitle"></div>' +
                '<div class="recipe-quick-actions-list" id="recipe-quick-actions-list"></div>' +
            '</div>';
            document.body.appendChild(overlay);
            return overlay;
        }

        function closeRecipeQuickActions() {
            const overlay = document.getElementById('recipe-quick-actions-overlay');
            if (overlay) overlay.classList.remove('active');
        }

        function openRecipeQuickActions(recipeId, event = null) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const recipe = getRecipeById(recipeId);
            if (!recipe) return showToast('Рецепт не найден');
            const overlay = ensureRecipeQuickActionSheet();
            const mealType = getSmartDefaultMealType(recipe.category || 'Обед');
            const lastMealType = getLastSmartMealType(mealType);
            setText('recipe-quick-actions-title', recipe.title || 'Рецепт');
            setText('recipe-quick-actions-subtitle', 'Быстрое действие без открытия карточки');
            const idArg = escapeAttr(JSON.stringify(String(recipe.id)));
            const list = document.getElementById('recipe-quick-actions-list');
            if (list) {
                list.innerHTML =
                    '<button type="button" onclick="closeRecipeQuickActions(); quickAddRecipeToDiary(' + idArg + ', null, \'serving\')"><b>Добавить 1 порцию</b><span>' + escapeHTML(mealType) + '</span></button>' +
                    '<button type="button" onclick="closeRecipeQuickActions(); quickAddRecipeToDiary(' + idArg + ', null, \'half\')"><b>Добавить 1/2 порции</b><span>' + escapeHTML(mealType) + '</span></button>' +
                    '<button type="button" onclick="closeRecipeQuickActions(); quickAddRecipeToDiary(' + idArg + ', null, \'last-meal\')"><b>В последний приём пищи</b><span>' + escapeHTML(lastMealType) + '</span></button>' +
                    '<button type="button" onclick="closeRecipeQuickActions(); addRecipeToDiary(' + idArg + ')"><b>Настроить граммы</b><span>Открыть полный add flow</span></button>';
            }
            overlay.classList.add('active');
        }

        function getDiaryMealSelectionKey(source, id) {
            return String(source || 'product') + ':' + String(id || '');
        }

        function normalizeDiaryMealSelectionProduct(product, source) {
            const mapped = mapSearchProduct(product, product?.source || source || 'search');
            if (!mapped) return null;
            return {
                id: product.id || product.searchId || mapped.id || '',
                searchId: product.searchId || '',
                source: source || product.source || 'search',
                name: mapped.name || product.name || 'Продукт',
                brand: product.brand || '',
                barcode: normalizeBarcode(product.barcode || ''),
                caloriesPer100: Number(mapped.caloriesPer100) || 0,
                proteinPer100: Number(mapped.proteinPer100) || 0,
                fatPer100: Number(mapped.fatPer100) || 0,
                carbsPer100: Number(mapped.carbsPer100) || 0,
                defaultGrams: Number(product.defaultGrams || product.grams || product.servingGrams) || 100
            };
        }

        function getDiaryMealSelectionProduct(source, id) {
            if (source === 'manual') {
                const product = loadManualProducts().find(item => String(item.id) === String(id));
                return product ? normalizeDiaryMealSelectionProduct(product, 'manual') : null;
            }
            if (source === 'search') {
                const product = getDiaryFoodSearchResult(id);
                return product ? normalizeDiaryMealSelectionProduct(product, 'search') : null;
            }
            return null;
        }

        function isDiaryMealProductSelected(source, id) {
            return diaryMealSelectedProducts.has(getDiaryMealSelectionKey(source, id));
        }

        function renderDiaryMealSelectionToggle(source, id) {
            const sourceArg = escapeAttr(JSON.stringify(String(source || '')));
            const idArg = escapeAttr(JSON.stringify(String(id || '')));
            const selected = isDiaryMealProductSelected(source, id);
            return '<button class="diary-meal-select-btn' + (selected ? ' is-selected' : '') + '" type="button" aria-label="' + (selected ? 'Убрать из выбора' : 'Выбрать продукт') + '" aria-pressed="' + (selected ? 'true' : 'false') + '" onclick="toggleDiaryMealProductSelection(event, ' + sourceArg + ', ' + idArg + ')"><span></span></button>';
        }

        function renderDiaryMealSelectionBar() {
            const bar = document.getElementById('diary-meal-selection-bar');
            const screen = document.getElementById('diary-meal-screen');
            if (!bar) return;
            const count = diaryMealSelectedProducts.size;
            bar.hidden = count <= 0;
            screen?.classList.toggle('has-diary-selection', count > 0);
            if (count <= 0) {
                bar.innerHTML = '';
                return;
            }
            bar.innerHTML = '<div class="diary-meal-selection-count">Выбрано: ' + count + '</div>' +
                '<button class="diary-meal-selection-clear" type="button" onclick="clearDiaryMealSelection()">Очистить</button>' +
                '<button class="diary-meal-selection-add" type="button" onclick="addSelectedDiaryMealProducts(event)">Добавить</button>';
        }

        function toggleDiaryMealProductSelection(event, source, id) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            const key = getDiaryMealSelectionKey(source, id);
            if (diaryMealSelectedProducts.has(key)) {
                diaryMealSelectedProducts.delete(key);
            } else {
                const product = getDiaryMealSelectionProduct(source, id);
                if (!product) return showToast('Продукт не найден');
                diaryMealSelectedProducts.set(key, { key, product });
            }
            renderDiaryMealContent();
        }

        function clearDiaryMealSelection() {
            diaryMealSelectedProducts.clear();
            renderDiaryMealSelectionBar();
            renderDiaryMealContent();
        }

        function getDiaryMealProductOpenAction(source, id) {
            const sourceArg = escapeAttr(JSON.stringify(String(source || 'manual')));
            const idArg = escapeAttr(JSON.stringify(String(id || '')));
            return 'startDiaryMealProductAddPress(event, ' + sourceArg + ', ' + idArg + ')';
        }

        function getDiaryMealQuickAddAction(source, id) {
            const sourceArg = escapeAttr(JSON.stringify(String(source || 'manual')));
            const idArg = escapeAttr(JSON.stringify(String(id || '')));
            return 'if(consumeQuickAddLongPress()) return; quickAddDiaryMealProduct(event, ' + sourceArg + ', ' + idArg + ')';
        }
        ...
        function renderDiaryMealFavoriteButton(type, id) {
            ...
            return '<button class="diary-meal-fav-btn' + (active ? ' active' : '') + '" type="button" aria-label="' + (active ? 'Убрать из избранного' : 'Добавить в избранное') + '" onclick="toggleDiaryMealFavorite(event, ' + escapeAttr(JSON.stringify(type)) + ', ' + idArg + ')">' + (active ? '♥' : '♡') + '</button>';
        }
        ...
        function renderDiaryMealProductCard(product) {
            const idArg = escapeAttr(JSON.stringify(String(product.id || '')));
            const isSelected = isDiaryMealProductSelected('manual', product.id);
            return '<article class="diary-meal-product-card' + (isSelected ? ' is-selected' : '') + '">' +
                renderDiaryMealSelectionToggle('manual', product.id) +
                ...
                '<button class="diary-meal-product-add" type="button" aria-label="Быстро добавить продукт" onpointerdown="' + getDiaryMealProductOpenAction('manual', product.id) + '" onpointerup="finishQuickAddPress()" onpointercancel="finishQuickAddPress()" onpointerleave="finishQuickAddPress()" onclick="' + getDiaryMealQuickAddAction('manual', product.id) + '">+</button>' +
            '</article>';
        }

        function renderDiaryMealSearchProductCard(product) {
            const idArg = escapeAttr(JSON.stringify(String(product.searchId || '')));
            const isSelected = isDiaryMealProductSelected('search', product.searchId);
            return '<article class="diary-meal-product-card diary-meal-search-card' + (isSelected ? ' is-selected' : '') + '">' +
                renderDiaryMealSelectionToggle('search', product.searchId) +
                '<button class="diary-meal-product-main" type="button" onclick="openDiaryMealSearchResult(' + idArg + ')">' +
                    '<b>' + escapeHTML(product.name || 'Продукт') + '</b>' +
                    '<span>' + escapeHTML(formatDiaryMealProductMacros(product)) + '</span>' +
                '</button>' +
                '<span class="diary-meal-search-spacer" aria-hidden="true"></span>' +
                '<button class="diary-meal-product-add" type="button" aria-label="Быстро добавить продукт" onpointerdown="' + getDiaryMealProductOpenAction('search', product.searchId) + '" onpointerup="finishQuickAddPress()" onpointercancel="finishQuickAddPress()" onpointerleave="finishQuickAddPress()" onclick="' + getDiaryMealQuickAddAction('search', product.searchId) + '">+</button>' +
            '</article>';
        }

        function getDiaryMealProductList() {
            const rawQuery = normalizeManualProductName(document.getElementById('diary-meal-search-input')?.value || '');
            const query = rawQuery.length >= FOOD_SEARCH_MIN_QUERY_LENGTH ? rawQuery : '';
            const products = getSortedManualProducts();
            const filtered = query ? products.filter(product => normalizeManualProductName(product.name).includes(query)) : products;
            if (diaryMealActiveFilter === 'Избранные') {
                const favs = getDiaryMealFavorites('product');
                return filtered.filter(product => favs.includes(String(product.id)));
            }
            if (diaryMealActiveFilter === 'Частые') {
                const usage = getProductUsageMap();
        ...
        function renderDiaryMealProductSearch() {
            const state = diaryFoodSearchState;
            if (state.status === 'loading') {
                return '<div class="diary-meal-empty diary-meal-search-state"><b>Ищу продукты...</b><span>Проверяю мои продукты, базу MyPie и Open Food Facts.</span></div>';
            }
            if (!state.results.length) {
                return '<div class="diary-meal-empty diary-meal-search-state"><b>Продукт не найден. Добавьте вручную.</b><span>Заполните КБЖУ один раз, и продукт сохранится в ваших продуктах.</span><button class="diary-meal-empty-action" type="button" onclick="openDiaryMealCreateProduct()">Добавить вручную</button></div>';
            }
            return '<div class="diary-meal-product-list">' + state.results.map(renderDiaryMealSearchProductCard).join('') + '</div>';
        }

        function renderFastAccessBlocks(scope = 'products') {
            const buttons = [];
            const recentProductId = getSmartRecentProductId();
            const frequentProductId = getSmartFrequentProductId();
            const frequentRecipeId = getMostUsedRecipeId();
            if (scope !== 'recipes' && recentProductId) {
                const product = loadManualProducts().find(item => String(item.id) === String(recentProductId));
                if (product) buttons.push('<button type="button" onclick="quickAddDiaryMealProduct(event, \'manual\', ' + escapeAttr(JSON.stringify(String(product.id))) + ')"><b>Недавно</b><span>' + escapeHTML(product.name || 'Продукт') + '</span></button>');
            }
            if (scope !== 'recipes' && frequentProductId && frequentProductId !== recentProductId) {
                const product = loadManualProducts().find(item => String(item.id) === String(frequentProductId));
                if (product) buttons.push('<button type="button" onclick="quickAddDiaryMealProduct(event, \'manual\', ' + escapeAttr(JSON.stringify(String(product.id))) + ')"><b>Часто</b><span>' + escapeHTML(product.name || 'Продукт') + '</span></button>');
            }
            if (scope !== 'products' && frequentRecipeId) {
                const recipe = getRecipeById(frequentRecipeId);
                if (recipe) buttons.push('<button type="button" onclick="quickAddRecipeToDiary(' + escapeAttr(JSON.stringify(String(recipe.id))) + ', event)"><b>Частый рецепт</b><span>' + escapeHTML(recipe.title || 'Рецепт') + '</span></button>');
            }
            buttons.push('<button type="button" onclick="repeatPreviousMealType(event)"><b>Повторить</b><span>Прошлый ' + escapeHTML(getDiaryMealType().toLowerCase()) + '</span></button>');
            return '<div class="fast-access-block" aria-label="Быстрые действия">' + buttons.join('') + '</div>';
        }

        async function repeatPreviousMealType(event) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            if (isAddingMeal) return;
            const mealType = getDiaryMealType();
            ...
            isAddingMeal = true;
            showToast('Повторяю ' + mealType.toLowerCase() + '...');
            try {
                const meals = await callServer('getMeals', { startDate: start.toISOString(), endDate: end.toISOString() });
                const source = (Array.isArray(meals) ? meals : []).filter(meal => getValidMealType(meal.meal_type || meal.mealType || 'Перекус', 'Перекус') === mealType);
                if (!source.length) {
                    showToast('Вчера такого приёма пищи не было');
                    return;
                }
                const createdAt = selectedDateTimeISO();
                for (const meal of source) {
                    const payload = {
                        recipe_id: meal.recipe_id || meal.recipeId || null,
                        name: meal.name || meal.recipes?.title || meal.recipe_title || 'Повтор',
                        kcal: Number(meal.kcal ?? meal.calories ?? meal.total_kcal) || 0,
                        protein: Number(meal.protein ?? meal.total_protein) || 0,
                        fat: Number(meal.fat ?? meal.total_fat) || 0,
                        carbs: Number(meal.carbs ?? meal.total_carbs) || 0,
                        meal_type: mealType,
                        created_at: createdAt,
                ...
                await refreshAllData();
                if (isDiaryMealScreenOpen()) renderDiaryMealContent();
                showToast('Повторено: ' + source.length);
            } catch (error) {
                console.error('Ошибка повтора приема пищи:', error);
                showToast('Не удалось повторить: ' + error.message);
            } finally {
                isAddingMeal = false;
            }
        }

        function renderDiaryMealInlineSourceTabs() {
            return '<div class="diary-meal-inline-bottom-tabs diary-meal-bottom-tabs" role="tablist" aria-label="Источник добавления">' +
                '<button class="' + (diaryMealSourceTab === 'library' ? 'active' : '') + '" type="button" data-diary-meal-source="library" onclick="setDiaryMealSourceTab(\'library\')"><span aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M5 5.5h14v13H5z"></path><path d="M8.5 5.5v13M15.5 5.5v13"></path><path d="M11.5 8.5h1M11.5 12h1M11.5 15.5h1"></path></svg></span>Библиотека</button>' +
                '<button class="' + (diaryMealSourceTab === 'create' ? 'active' : '') + '" type="button" data-diary-meal-source="create" onclick="setDiaryMealSourceTab(\'create\')"><span aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M7 4.5h8.5l2.5 2.5v12.5H7z"></path><path d="M15.5 4.5V7H18"></path><path d="M10 10h5M10 13h5"></path><path d="M17.5 15.5h4M19.5 13.5v4"></path></svg></span>Создать</button>' +
            '</div>';
        }
        ...
        function renderDiaryMealRecipes() {
            ...
            let catalogItems = applySmartRecipeOrdering(sortRecipeItems(getEnrichedRecipes()
            ...
            const ownRecipeSource = query
                ...
                : allRecipes;
            const recipes = diaryMealRecipeFilter === 'Каталог' ? [] : diaryMealRecipeFilter === 'Избранное'
            ...
            const catalogHtml = catalogItems.length
                ? '<section class="diary-meal-recipe-section"><div class="diary-meal-section-title">Рецепты из приложения</div><div class="recipes-results-grid recipe-grid diary-meal-catalog-grid">' + renderRecipeGrid(catalogItems, favs, { onClick: 'quickAddRecipeToDiary' }) + '</div></section>'
                : '';
            const ownEmpty = !allRecipes.length && !query
                ? '<div class="diary-meal-empty diary-meal-own-empty"><b>Пока нет собственных рецептов.</b><span>Создай рецепт, и он появится здесь.</span><button class="diary-meal-empty-action" type="button" onclick="openDiaryMealCreateRecipe()">Создать рецепт</button></div>'
                : '';
            const ownHtml = recipes.length
                ? '<section class="diary-meal-recipe-section"><div class="diary-meal-section-title">Собственные рецепты</div><div class="recipes-results-grid recipe-grid diary-meal-catalog-grid">' + renderMyRecipeGrid(recipes, { onClick: 'quickAddMyRecipeToDiary' }) + '</div></section>'
                : ownEmpty;
            if (!catalogHtml && !ownHtml) {
                return '<div class="diary-meal-empty"><b>Рецептов пока нет.</b><span>Создай свой рецепт или измени фильтр.</span><button class="diary-meal-empty-action" type="button" onclick="openDiaryMealCreateRecipe()">Создать рецепт</button></div>';
            }
            return renderFastAccessBlocks('recipes') + catalogHtml + ownHtml;
        }
        ...
        function renderDiaryMealContent() {
            ...
            if (hasFoodSearchQuery()) {
                const query = normalizeFoodSearchText(document.getElementById('diary-meal-search-input')?.value || '');
                if (query !== diaryFoodSearchState.query) handleDiaryMealSearchInput();
                content.innerHTML = renderDiaryMealProductSearch();
                syncFooter();
                return;
            }
            const products = getDiaryMealProductList();
            if (!products.length) {
                const hasQuery = normalizeManualProductName(document.getElementById('diary-meal-search-input')?.value || '').length >= FOOD_SEARCH_MIN_QUERY_LENGTH;
            ...
            content.innerHTML = renderFastAccessBlocks('products') +
                '<div class="diary-meal-date">' + new Date(currentDate).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' }) + '</div>' +
        ...
        async function addDiaryMealProduct(event, productId) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            return quickAddDiaryMealProduct(event, 'manual', productId);
        }
        ...
        function openDiaryMealProduct(productId) {
            currentMealFilter = currentDiaryMealType;
            const product = loadManualProducts().find(item => String(item.id) === String(productId));
            const mapped = mapSearchProduct(product, 'manual');
            if (!mapped) return showToast('Продукт не найден');
            openProductSearchMealModal(mapped);
        }

        async function quickAddDiaryMealProduct(event, source, id) {
            event?.preventDefault?.();
            event?.stopPropagation?.();
            if (isAddingMeal) return;
            const product = getDiaryMealSelectionProduct(source, id);
            ...
            isAddingMeal = true;
            showToast('Добавляю в ' + mealType + '...');
            const optimisticId = addOptimisticDiaryRow({ name: product.name, k: payload.kcal, p: payload.protein, f: payload.fat, c: payload.carbs }, mealType);
            try {
                try {
                    await callServer('addMeal', payload);
                } catch (error) {
                ...
                recordProductSmartAction(product.id || product.searchId || product.name, mealType);
                await refreshAllData();
                if (isDiaryMealScreenOpen()) renderDiaryMealContent();
                document.getElementById('history-list')?.classList.add('success-flash');
                setTimeout(() => document.getElementById('history-list')?.classList.remove('success-flash'), 700);
                ...
                removeOptimisticDiaryRow(optimisticId);
                console.error('Ошибка быстрого добавления продукта:', error);
                showToast('Не удалось добавить продукт: ' + error.message);
            } finally {
                isAddingMeal = false;
            }
        }
        ...
        function openDiaryMealBarcode() {
            currentMealFilter = currentDiaryMealType;
            openBarcodeMealModal({ mealType: currentDiaryMealType, startCamera: true });
        }
        ...
        function openDiaryFoodPickerBarcode(event) {
            ...
            openBarcodeMealModal({ mealType, startCamera: true });
        }

        function diaryMealCollapsedStorageKey() {
            return 'diaryMealCollapsed_' + appUserId;
        }

        function getCollapsedDiaryMeals() {
            try { return new Set((JSON.parse(localStorage.getItem(diaryMealCollapsedStorageKey())) || []).map(String)); } catch (e) { return new Set(); }
        }

        function setDiaryMealCollapsed(type, collapsed) {
            const collapsedMeals = getCollapsedDiaryMeals();
            if (collapsed) collapsedMeals.add(String(type));
            else collapsedMeals.delete(String(type));
            localStorage.setItem(diaryMealCollapsedStorageKey(), JSON.stringify(Array.from(collapsedMeals)));
        }

        function getDiaryMealIcon(type) {
            const icons = { 'Завтрак': '☀', 'Обед': '◐', 'Ужин': '☾', 'Перекус': '◆' };
            return icons[type] || '•';
        }
        ...
        function mealSupportsWeightEdit(meal) {
            return Number(meal?.grams) > 0 &&
                Number(meal?.calories_per_100) >= 0 &&
                Number(meal?.protein_per_100) >= 0 &&
                Number(meal?.fat_per_100) >= 0 &&
                Number(meal?.carbs_per_100) >= 0;
        }
        ...
        async function updateHistoryUI(options = {}) {
            ...
            diaryMealsCache = meals;
            const html = ['Завтрак', 'Обед', 'Ужин', 'Перекус']
                .map(type => renderDiaryMealGroup(type, meals.filter(m => (m.meal_type || m.mealType || 'Перекус') === type)))
                .join('');
            hList.innerHTML = '<div class="diary-existing-meals">' + html + '</div>';
        }

        function toggleMealGroup(el, type) {
            const group = el?.closest?.('.meal-group');
            const content = group?.querySelector('.meal-group-content');
            ...
            setDiaryMealCollapsed(type, collapsed);
        }

        function replaceDiaryMealItem(mealType) {
            openDiaryFoodPickerScreen(getValidMealType(mealType, 'Перекус'), null, { activeTab: 'products' });
        }

        async function editDiaryMealWeight(id) {
            const meal = diaryMealsCache.find(item => String(item.id) === String(id));
            if (!meal) return showToast('Запись не найдена');
            if (!mealSupportsWeightEdit(meal)) return showToast('Для этой записи вес нельзя изменить автоматически');
            const currentGrams = Math.round(Number(meal.grams) || 100);
            const nextRaw = prompt('Новый вес, г', String(currentGrams));
            if (nextRaw === null) return;
            const grams = Number(String(nextRaw).replace(',', '.'));
            if (!Number.isFinite(grams) || grams <= 0) return showToast('Вес должен быть больше 0 г');
            const ratio = grams / 100;
            const mealType = getValidMealType(meal.meal_type || meal.mealType || 'Перекус', 'Перекус');
            const createdAt = meal.created_at || meal.createdAt || selectedDateTimeISO();
            const payload = {
                type: meal.type || 'manual-product',
                manual: meal.manual ?? true,
                productId: meal.productId || meal.product_id || null,
                name: getDiaryMealTitle(meal),
                recipe_title: getDiaryMealTitle(meal),
                mealType,
                meal_type: mealType,
                grams,
                barcode: meal.barcode || '',
                brand: meal.brand || '',
                calories_per_100: Number(meal.calories_per_100) || 0,
                protein_per_100: Number(meal.protein_per_100) || 0,
                fat_per_100: Number(meal.fat_per_100) || 0,
                carbs_per_100: Number(meal.carbs_per_100) || 0,
                calories: (Number(meal.calories_per_100) || 0) * ratio,
                kcal: (Number(meal.calories_per_100) || 0) * ratio,
                protein: (Number(meal.protein_per_100) || 0) * ratio,
                fat: (Number(meal.fat_per_100) || 0) * ratio,
                carbs: (Number(meal.carbs_per_100) || 0) * ratio,
                recipe_id: null,
                createdAt,
                created_at: createdAt,
                ingredients: []
            };
            await callServer('deleteMeal', { id });
            await callServer('addMeal', payload);
            await refreshAllData();
            showToast('Вес обновлён');
        }

                                function openRecipeDetails(id) {
            console.log('OPEN_RECIPE_DETAIL', id);
            const r = recipesData.find(x => String(x.id) === String(id));
            if (!r) return;
            const nutrition = getRecipeNutrition(r);
            const img = safeImageUrl(r.image_url || r.image || '');
            recipeDetailsCurrentId = String(r.id);

            const page = document.getElementById('recipe-view-page');
            const photoWrap = document.getElementById('recipe-view-photo-wrap');
            const image = document.getElementById('recipe-view-img');
            const placeholder = document.getElementById('recipe-view-photo-placeholder');
            const title = document.getElementById('recipe-view-title');
            const description = document.getElementById('recipe-view-description');
            const kbju = document.getElementById('recipe-view-kbju');
            const facts = document.getElementById('recipe-view-facts');
            const ingredients = document.getElementById('recipe-view-ingredients');
            const instructions = document.getElementById('recipe-view-instructions');
            const addButton = document.getElementById('recipe-view-add-btn');
            const workingIngredients = getRecipeWorkingIngredients(r);
            const total = getRecipePortionNutrition(r, workingIngredients);
            const servings = getRecipeServings(r);
            const outputWeight = Math.round(Number(total.grams) || Number(nutrition.grams) || 0);

            syncRecipeDetailsFavoriteButton(r.id);

            if (title) title.textContent = r.title || r.name || 'Рецепт';
            if (description) description.textContent = r.description || 'Описание рецепта пока не добавлено.';
            if (kbju) {
                kbju.textContent = Math.round(Number(nutrition.kcal) || 0) + ' ккал • Б ' +
                    (Number(nutrition.protein) || 0).toFixed(1) + ' • Ж ' +
                    (Number(nutrition.fat) || 0).toFixed(1) + ' • У ' +
                    (Number(nutrition.carbs) || 0).toFixed(1);
            }
            if (facts) {
                const weightIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 8h10l1.5 11h-13L7 8Z"></path><path d="M9 8a3 3 0 0 1 6 0"></path><path d="M12 13h.01"></path></svg>';
                const portionIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="7"></circle><path d="M4 12h16"></path><path d="M12 4v16"></path></svg>';
                const factsItems = [
                    outputWeight > 0 ? '<span>' + weightIcon + outputWeight + ' г</span>' : '',
                    servings > 0 ? '<span>' + portionIcon + servings + ' ' + (servings === 1 ? 'порция' : 'порций') + '</span>' : ''
                ].filter(Boolean);
                facts.innerHTML = factsItems.join('');
                facts.hidden = !factsItems.length;
            }
            ...
            photoWrap?.classList.toggle('is-empty', !img);
            if (placeholder) placeholder.hidden = Boolean(img);

            if (ingredients) {
                const visibleIngredients = workingIngredients.filter(ingredient => Number(ingredient.weight || ingredient.default_grams || ingredient.defaultGrams || ingredient.grams) > 0);
                ingredients.innerHTML = visibleIngredients.length
                    ? visibleIngredients.map(ingredient => {
                        const name = ingredient.products?.name || ingredient.name || 'Ингредиент';
                        const grams = Number(ingredient.weight || ingredient.default_grams || ingredient.defaultGrams || ingredient.grams) || 0;
                        return '<div class="ingredient-item"><span>' + escapeHTML(name) + '</span><span>' + Math.round(grams) + ' г</span></div>';
                    }).join('')
                    : '<p class="recipe-view-empty">Ингредиенты не указаны.</p>';
            }

            if (instructions) {
                const steps = getRecipeInstructionSteps(r);
                instructions.innerHTML = steps.length
                    ? steps.map((step, index) => '<div class="detail-step"><span>' + (index + 1) + '</span><p>' + escapeHTML(step) + '</p></div>').join('')
                    : '<p class="recipe-view-empty">Инструкция по приготовлению пока не добавлена.</p>';
            }
            ...
            if (addButton) {
                addButton.onclick = (event) => {
                    event.preventDefault();
                    event.stopPropagation();
                    addRecipeToDiary(r.id);
                };
            }

            setDisplayedLayer('recipe-view-page', page, true);
        }

        function closeRecipeDetails() {
            setDisplayedLayer('recipe-view-page', document.getElementById('recipe-view-page'), false);
            setDisplayedLayer('recipe-details', document.getElementById('recipe-details-modal'), false);
            recipeDetailPortionDraft = null;
            recipeDetailsCurrentId = null;
        }

        window.openRecipeDetails = openRecipeDetails;
        window.closeRecipeDetails = closeRecipeDetails;

        function addOptimisticDiaryRow(meal, mealType) {
            const list = document.getElementById('history-list');
            if (!list || !meal) return '';
            const id = 'optimistic-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7);
            const recipe = getRecipeById(meal.rid);
            const title = recipe?.title || meal.name || 'Добавляется';
            const row = document.createElement('div');
            row.className = 'history-item diary-food-row optimistic';
            row.dataset.optimisticId = id;
            row.innerHTML = '<div class="diary-food-row-main"><div class="diary-food-row-title">' + escapeHTML(title) + '</div><div class="diary-food-row-meta">' + Math.round(Number(meal.k) || 0) + ' ккал • ' + escapeHTML(mealType) + ' • синхронизация</div></div>';
            const container = list.querySelector('.diary-existing-meals') || list;
            container.prepend(row);
            return id;
        }

        function removeOptimisticDiaryRow(id) {
            if (!id) return;
            Array.from(document.querySelectorAll('[data-optimistic-id]')).find(row => String(row.dataset.optimisticId) === String(id))?.remove();
        }

        function openMealModal(rid, k, p, f, c, ingredients = [], mealType = null, context = {}) {
            console.log('openMealModal', { rid, k, p, f, c, mealType });
            pendingMeal = { rid, k, p, f, c, ingredients: snapshotPortionIngredients(ingredients), context };
        ...
        async function confirmMealAdd(mealType) {
            ...
            const optimisticId = addOptimisticDiaryRow(meal, mealType);
            try {
                console.log('addMeal request', mealType, { ...meal, createdAt });
                if (window.DEBUG_ADD_MEAL_ALERT) alert('addMeal request: ' + mealType);
                await callServer('addMeal', { recipe_id: meal.rid, kcal: meal.k, protein: meal.p, fat: meal.f, carbs: meal.c, meal_type: mealType, created_at: createdAt, ingredients: meal.ingredients });
                await saveRecipeDiarySnapshot(meal, mealType).catch(error => console.warn('Не удалось сохранить состав рецепта в diary_entries:', error));
                if (meal.rid) recordRecipeSmartAction(meal.rid, mealType, meal.context?.action || 'serving');
        ...
        async function deleteOneMeal(id) {
            if (!confirm('Удалить запись из дневника?')) return;
            const row = Array.from(document.querySelectorAll('.diary-food-row')).find(item => String(item.dataset.mealId) === String(id));
            row?.classList.add('is-removing');
            if (row) await new Promise(resolve => setTimeout(resolve, 180));
            await callServer('deleteMeal', { id });
            await refreshAllData();
        }
